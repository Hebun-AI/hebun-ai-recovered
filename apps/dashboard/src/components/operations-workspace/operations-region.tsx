...
/*
 * Shared chrome + honesty primitives for the Operations Workspace (Phase 10).
 *
 * Live, temporal, state-driven identity — distinct from Intelligence (analytical)
 * and Knowledge (reference). Uses the published design system. Every absent state
 * is explained: empty is honest, and unavailable is never shown as healthy.
 */

export function OperationsRegion({
  title,
  eyebrow,
  action,
  children,
...
/** Calm marker that the operational view is a non-authoritative system view. */
export function SystemViewMarker({ label = "System view" }: { label?: string }) {
  return (
