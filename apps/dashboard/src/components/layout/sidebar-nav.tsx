...
const destinations = [
  { label: "Overview", href: "/director", icon: Gauge },
  ...
  { label: "Risk & Compliance", href: "/director/governance/risk", icon: ShieldCheck },
] as const;
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <nav aria-label="Primary" className="flex flex-1 flex-col gap-1 overflow-y-auto px-3 py-4">
        <p className="px-3 pb-2 text-[0.68rem] font-semibold uppercase tracking-[0.16em] text-fg-muted">Workspace</p>
        {destinations.map(({ label, icon: Icon, ...destination }) => {
