...
async function backfillProof(): Promise<void> {
  ...
  assert.equal(journal.entries.length, 66, "MV-2 authored exactly one migration: 63 -> 64"); /* MV-5: ledger 65 -> 66 (mp4-normalize-v1 derivation CHECKs on media_assets, additive). */ /* MV-4: ledger 64 -> 65. */
