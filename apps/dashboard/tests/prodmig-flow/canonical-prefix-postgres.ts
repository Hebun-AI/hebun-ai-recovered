...
function theCanonicalLedgerIsWellFormed(): void {
  /*
   * THE NUMBER AND THE SENTENCE MUST AGREE. This read "authors 47 canonical migrations" while
   * asserting 49: two earlier phases moved the value and left the prose behind, so the message a
   * failure would have printed was already false. It is stated once, from the constant, so the
   * two can no longer drift apart.
   */
  assert.equal(
    CANONICAL.length,
    66, /* MV-5: ledger 65 -> 66 (mp4-normalize-v1 derivation CHECKs on media_assets, additive). */ /* MV-4: ledger 64 -> 65 (async generation lifecycle on media_generation_invocations, additive). */ /* MV-2: 63 -> 64. */
    `this checkout authors ${CANONICAL.length} canonical migrations, and the pin expected 66`,
  ...
  assert.equal(
    ...
    /* MV-5 holds the newest line now; MV-4 held it before. */ "20260926145724_mv5_video_derivation",
   ...
   * moves again: `8394a8f461cdc9a9bf7dac9e13af8192` was the digest at 47. TRH-19 authored 49 — one
   * additive column on `heby_action_requests` plus its two CHECKs — and moves it once more:
   * `f11fb805ef8d822e4a59226e4600404e` was the digest at 48. TRH-21 authored 50 — one additive
   * `provider_observations` table with its composite tenant/connection foreign key — and moves it
   * again: `3054a933444257110870a773d5afe4ce` was the digest at 49. Production still
   * carries the digest for ITS ledger until the migration ceremony is authorized — the gap is a
   * TRH-23 authored 51 — one additive `standing_observation_authorizations` table plus the
   * `standing-observation` governance domain — and moves it again:
   * `76d35e01e43a609029bcd2cb86152282` was the digest at 50. Production still
   * carries the digest for ITS ledger until the migration ceremony is authorized — the gap is a
   * TRH-24 authored 52 — the machine-observation provenance evolution on `provider_observations`,
   * which drops two NOT NULL constraints and adds the XOR provenance check — and moves it again:
   * `960f7d53c126d67fdaf9cda30cc3c405` was the digest at 51. Production still
   * carries the digest for ITS ledger until the migration ceremony is authorized — the gap is a
   * PENDING ROLLOUT, which is exactly what this assertion exists to make visible.
   */
  assert.equal(canonicalDigest(CANONICAL), "51a288a38d93d55448491ae797ca83a0", "the release digest"); /* MV-5: 65 -> 66 appended; the first 65 files still digest to the MV-4 values. */ /* MV-4: 64 -> 65 appended; the first 64 files still digest to the MV-2 values. */ /* MV-2: one appended migration (63 -> 64, none edited) moved the release digest from 71058586…. */ /* MV-0: six appended migrations (57 -> 63, none edited) moved the release digest from 91ea9382…; the disposable-database assertions below follow it. */ /* MEDIA-2A 56 -> 57; production stands at 56 (`3296764e10a243fa621c03b0bfd3cbd7`) until its gated ceremony runs. */
  assert.equal(
...
async function aTargetOneBehindAppliesOnlyTheLast(): Promise<void> {
  ...
  try {
    await withDatabase("prodmig_behind1", async (client) => {
      ...
      assert.equal(verdict.finalDigest, "51a288a38d93d55448491ae797ca83a0");
       ...
       *
       * TRH-19 RETURNS THE PROBE TO pg_constraint, because that is what this release adds: one
       * additive column on `heby_action_requests` and two CHECKs. TRH-10's `artifact-review` enum
       * member is now added by an EARLIER migration and would be satisfied before migrating,
       * proving nothing about the pending one; `heby_action_requests_agent_rationale_chk` is what
       * this release adds and nothing earlier could have.
       *
       * TRH-24 MOVES THE PROBE AGAIN — AND CHANGES ITS SHAPE, WHICH IS THE POINT.
       * `standing_observation_authorizations` is now created by an EARLIER migration and would be
       * satisfied before migrating. TRH-24 creates NO TABLE at all: it evolves an existing one. So
       * the probe becomes a COLUMN — `provider_observations.standing_authorization_id` — which is
       * what THIS release adds and which no earlier migration mentions.
       *
      ...
      const admitsSelfService = async (): Promise<string> => {
        const r = await client.query<{ n: string }>(
          ...
          `select count(*)::text as n from pg_constraint
            where conname = 'media_assets_derivation_mp4_chk'` /* MV-5 is the pending migration now and adds no column; probe ITS constraint. */,
        );
        return r.rows[0]!.n;
      };
      assert.equal(
      ...
      assert.equal(after.applied, 66); /* MV-5: ledger 65 -> 66 (mp4-normalize-v1 derivation CHECKs on media_assets, additive). */ /* MV-4: ledger 64 -> 65 (async generation lifecycle on media_generation_invocations, additive). */ /* MV-2: ledger 63 -> 64. */
      assert.equal(after.digest, "51a288a38d93d55448491ae797ca83a0");
...
async function aConvergedTargetIsANoOp(): Promise<void> {
  await withDatabase("prodmig_converged", async (client) => {
    ...
    assert.equal(verdict.applied, 66); /* MV-5: ledger 65 -> 66 (mp4-normalize-v1 derivation CHECKs on media_assets, additive). */ /* MV-4: ledger 64 -> 65 (async generation lifecycle on media_generation_invocations, additive). */ /* MV-2: ledger 63 -> 64. */
    assert.equal(verdict.digest, "51a288a38d93d55448491ae797ca83a0");
