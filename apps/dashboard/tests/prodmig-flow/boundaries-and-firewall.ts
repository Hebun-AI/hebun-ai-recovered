...
function itIsInfrastructureNotSchema(): void {
  ...
  assert.equal(files.length, 39, "the production migration AUTHORITY authors no migration of its own");
  const journal = JSON.parse(read("src/db/migrations/meta/_journal.json")) as { entries: unknown[] };
  assert.equal(journal.entries.length, 39, "and the journal agrees");
}
