...
function itIsInfrastructureNotSchema(): void {
  ...
  assert.equal(files.length, 43, "the production migration AUTHORITY authors no migration of its own"); /* WORK-1 grew the ledger 41 -> 42: the Organizational Work Authority table. */
  const journal = JSON.parse(read("src/db/migrations/meta/_journal.json")) as { entries: unknown[] };
  assert.equal(journal.entries.length, 43, "and the journal agrees");
}
