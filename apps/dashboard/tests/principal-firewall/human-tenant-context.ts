...
function main(): void {
   ...
   *
   * MEDIA-1 adds one, a permitted kind and not a grant. `db/schema/media-asset.ts` names
   * `agents.tenantId` as the second half of a composite foreign key, so a generation invocation
   * recording ANOTHER tenant's agent as its author is a DATABASE error. The Media Asset runtime reads
   * no agent row itself: it resolves the author through the released authorship seam.
   *
   * The census stays EXACT, which is the whole value of it: a twelfth file still fails here.
   */
  ...
  assert.deepEqual(
    ...
    'governance subject types are exactly ["knowledge_node", "work_artifact_revision", "media_asset"] — a principal cannot become a Governance subject',
  );
  assert.ok(
