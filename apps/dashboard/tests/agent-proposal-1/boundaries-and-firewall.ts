...
function schemaIsUntouched(): void {
   ...
   */
  assert.equal(sql.length, 47, "AGENT-PROPOSAL-1 adds no migration"); /* WEV-1 grew the ledger 44 -> 45; PBGA-1 45 -> 46; CGO-1 46 -> 47 (content-draft + destination). */
  const journal = JSON.parse(read("src/db/migrations/meta/_journal.json")) as {
    entries: readonly unknown[];
  };
  assert.equal(journal.entries.length, 47, "and the ledger is unchanged by this phase");
