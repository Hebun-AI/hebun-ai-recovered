...
function oneAgentLookupAuthority(): void {
   ...
   *
   * MV-4 adds the asynchronous path of the SAME generation authority: a video invocation records its
   * authoring agent exactly as an image invocation does. Same use, same seam; the census stays exact.
   */
  ...
  assert.deepEqual(
    resolvers.sort(),
    [SEAM, "src/features/media-assets/request-media-generation.server.ts", "src/features/media-assets/async-generation-lifecycle.server.ts"].sort(),
