...
function main(): void {
  ...
  {
    assert.deepEqual(GOVERNANCE_SUBJECT_TYPES, ["knowledge_node", "work_artifact_revision", "media_asset"]);
  ...
  /* ── T7: THE AUDIT SINK HAS FIVE DECLARED OWNERS, NO MORE ───────────────── */
  {
    ...
    assert.deepEqual(
      candidates
        ...
        .sort(),
      AUDIT_SINK_WRITERS,
      "seven declared sibling owners write the sink — and nothing else does",
    );
