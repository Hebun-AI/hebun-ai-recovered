...
function main(): void {
  ...
  /* ── 8 · THE LEDGER DID NOT MOVE ─────────────────────────────────────────── */
  {
    ...
    assert.equal(journal.entries.length, 41, "A1a adds no migration — the ledger carries none of its authoring"); /* OSA-1 grew the ledger 40 -> 41: the departments additive hardening. */
  }
