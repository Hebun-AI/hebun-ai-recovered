...
function main(): void {
   ...
   */
  const agentIdMigrations = readdirSync(path.join(ROOT, MIGRATIONS)).filter(
    (f) => f.endsWith(".sql") && /agent[_-]?id/i.test(f),
  );
  assert.deepEqual(agentIdMigrations, [], "AGENT-ID-0 authored no migration — this phase needed none");
