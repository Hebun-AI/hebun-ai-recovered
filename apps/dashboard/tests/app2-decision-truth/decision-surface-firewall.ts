...
function main(): void {
   ...
   */
  {
    ...
    assert.ok(
      ...
      "while the proposer column stays open, so a real agent may propose one day",
    );
  }
  ...
  /* ── 16 · NO SCHEMA, NO MIGRATION ────────────────────────────────────────── */
  {
    ...
    assert.equal(journal.entries.length, 37, "APP-2 adds no migration — the ledger carries none of its authoring");
  }
