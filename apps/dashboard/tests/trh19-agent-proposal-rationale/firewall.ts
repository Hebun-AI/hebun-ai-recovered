...
function main(): void {
   ...
   * ═════════════════════════════════════════════════════════════════════ */
  {
    ...
    assert.equal(
      ...
      /* MEDIA-2A — the live image transport's provenance (`transport` CHECK widened to fake|live,
       * `provider_failure`, provider token usage), so this is now the newest canonical migration. */
      /* MV-5 holds the newest line now; MV-4 held it before. */ "20260926145724_mv5_video_derivation",
      "and the live image transport holds the newest line — the Media Asset authority held it before, RUNG 2's standing mutation authority before that, and TRH-19 before those",
    );
