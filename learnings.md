...
## ERA III OPENED + AGENT MANDATE BOUNDARY DISCOVERY — the previous conclusion was half right, and the half that was wrong was the ownership

- **"A mandate is just a Governance subject" is half a sentence, and the missing half is the whole design.** A mandate *change* must be a Governance subject; a mandate is not a Governance-owned fact. The way to see it was G6A's finding, already written down: **Governance authority resolves from `decision_records` and nothing else.** So anything stored in that table becomes a Governance-derived fact — which for a mandate means Governance silently becomes the workforce authority. **An existing finding about one table decided the ownership question for a capability nobody had designed yet.**
- **The repository had already answered it six times and said so out loud.** `decide-improvement-hypothesis.server.ts` names the pattern in its own header — *"the shape every other subsystem already uses to submit its own subject — decide-action-request, decide-enrollment, authorize-membership, ratify-version, provision-member-role"* — and states the split as `SIA-3 OWNS the hypothesis; GOVERNANCE OWNS the decision`. **Six precedents for the owner-owns-state shape, zero for Governance owning a subject's state.** Reading one sibling's header was worth more than reading the whole Governance feature.
- **The closed subject vocabulary was a red herring, and checking WHICH writer it guards is what dissolved it.** `GOVERNANCE_SUBJECT_TYPES` is `["knowledge_node"]` and looks like a hard wall. It guards only the generic human inlet. The transaction-joinable writer `writeGovernanceDecisionWithin` already accepts **eight** subject types, and `decision_records.subject_type` is `text`, so a ninth costs **no migration**. **Two layers, one name — the guard everyone quotes is not the guard in the path.**
- **The "second agent" acceptance condition was unreachable by arithmetic, not by policy.** `createDurableAgentIdentity` takes `lock table agents in share row exclusive mode`, counts with no lifecycle and no soft-delete predicate, and refuses above zero — retired rows count, retirement writes no DELETE. And `resolveAgentProposer` independently refuses `ambiguous-durable-agent-identity` above one in service. So a second agent needs a reopened one-way ceremony **and** a new selection design. **The proposed test would have required manufacturing two authorities; the cheapest way to find that out was to read the count predicate rather than the feature's prose.**
- **Today's mandate already exists — as a global constant.** `AGENT_ORIGINABLE_ACTION_KINDS = ["send"]`, identical for every tenant and every agent, with no writer. That reframes the whole program: it is not "add a constraint", it is "move an existing constraint from a compile-time constant to a tenant-owned, Governance-authorized record". **Find the constant that is already doing the job before designing the table that would replace it.**
- **The most dangerous column was the one with no writer and a live reader.** `agents.authority_ceiling` has zero writers — and `canonical-read/actor-resolution.ts` already summarizes it into `authority_ceiling_summary`. Storing a mandate there would publish a *constraint* as an *authority ceiling* through canonical actor resolution on the same deploy, with no test failing. **A dead column is only dead on the write side; check who reads it before reusing it.**
- **Opening an era and starting work on it are two acts, and the roadmap only permits the first here.** The temptation was to record Era III as open *and* Agent Mandate as the first program. The Director gated it, and the repository agrees: `CANDIDATE != PROGRAM`, `DISCOVERY COMPLETE != WORK STARTED`. The activation record supersedes the "Era III = NOT STARTED" line without rewriting it, exactly as §12.9 supersedes the per-milestone "Era II remains OPEN" lines.

**Weekly three.** *Learned:* the previous recommendation was inherited rather than proved, and both halves that broke — the owner and the acceptance test — broke against facts that were already written down in files nobody re-read. *Turkish Rug House:* nothing shipped, and that is correct — the company's one agent stays exactly as constrained as it was, and the question "what is this agent actually for, and who decided that" now has a designed answer instead of a hard-coded one. *Hebun AI:* zero source change, zero schema, zero migration, zero production row — ledger 39 — Era III is OPEN with **no milestone selected**, and the first candidate is recorded as a candidate whose implementation is still a Director decision.

## AMA-1 — the authority was easy; the censuses were the work

- **AGENT IDENTITY != AGENT MANDATE, and the repository had already made the second one impossible to fold into the first.** `features/agent-identity` states "TWO authorities, TWO transitions, and no third" and exports no update. So the question "should mandate be a column on `agents`?" was answered by a sentence somebody wrote a phase earlier — and the two other reasons (a mutable row cannot carry a supersession chain; `authority_ceiling` already has a summarizing reader) only confirmed it. **A boundary stated in a barrel's header is a design constraint, not documentation.**
- **The most dangerous place to put a mandate was the column with no writer.** `agents.authority_ceiling` is dead on the write side and alive on the read side — `canonical-read/actor-resolution.ts` summarizes it into `authority_ceiling_summary`. Storing a CEILING there would have published it as an AUTHORITY through canonical actor resolution on the same deploy, with no test failing. It is now banned by identifier in both casings across the feature, the schema, the audit sibling and the migration, and a bite-proof adds the reference and watches the firewall bite. **Check who READS a dead column before reusing it.**
- **A UNIQUE ordinal beats a partial index on an effective flag, because it removes the update entirely.** The discovery proposed `WHERE effective`; that would make every mandate change EDIT the previous row. `(tenant_id, agent_id, mandate_revision)` unique means effective = `max(revision)` by arithmetic, history is byte-identical forever, and the index doubles as the concurrency guarantee — so unlike the agent identity ceremony this writer needs **no table lock**. The identity ceremony's own comment explains why it needed one: *"`agents` carries NO unique index."* **The reason a precedent took a precaution is more portable than the precaution.**
- **`approve` is used by two subjects, and only the subject tells them apart.** Without an explicit subject check placed BEFORE the generic branches, a mandate decision would have recorded `membership-authorized` in the permanent ledger — an agent's purpose, filed as a human being admitted to the organization. SIA-3's comment warned about exactly this shape one phase earlier and I still had to look. **Read the fall-through order of a shared writer before adding a subject to it.**
- **The invariant "a mandate may only subtract" is a TYPE, not a check.** `MANDATE_SCOPE_VOCABULARY` is `AGENT_ORIGINABLE_ACTION_KINDS` — the same reference, not a copy — so a superset does not compile. The database repeats the list once in SQL because a CHECK cannot import TypeScript, and a test pins the two equal so the only possible drift direction is the database being NARROWER. **When a guarantee can be a type, the test's job is only to pin the one place the type cannot reach.**
- **The real cost of a migration is not the migration.** One table and one enum value took an afternoon; **26 absolute ledger pins, 3 audit-owner censuses, 2 human-only CHECK censuses, 13 migration-filename censuses, 3 journal-anchored bite-proofs, 2 ledger digests and 4 prodmig scenario counts** took longer than the feature. Every one was EXTENDED, never relaxed to "at least N" — which would let a future phase delete a released constraint and still pass. **A phase that adds a migration should budget for the census sweep, not be surprised by it.**
- **Three pins were repaired to be DERIVED rather than literal**, because each had already gone stale once: the newest-journal-entry index now follows `entries.length`, the target-ahead count is `CANONICAL.length + 1`, and the truncated-ledger count is `CANONICAL.length - 1`. **A literal that means "the last one" is a comment pretending to be an assertion.**
- **I killed a running suite and it left mutated source on disk.** A bite-proof was mid-mutation; `agent-proposal-request.tsx` came back with two swapped lines in a released file. Caught by `git status` before it could reach a commit, restored from HEAD, and the whole full-suite run was discarded as a verdict — I had been editing test files while it ran, which makes any result from it meaningless. **Never edit the tree during a full-suite run, and never kill one mid-flight; if you do, diff against HEAD before believing anything.**
- **AMA-1 records and does not enforce, and that absence is MEASURED rather than promised.** A census over all of `src/` proves exactly three modules outside the feature can see a mandate — the schema barrel, the audit sibling, and the Governance decision writer — and none is a proposal path. Every audit row carries `enforced: false`. **The cheapest way to keep a phase honest about what it did not do is to make the not-doing a failing test.**

**Weekly three.** *Learned:* the authority question was settled by the discovery, so the implementation risk moved somewhere unglamorous — the twenty-six places the repository counts its own migrations, and the two censuses that had to grow in the strict direction. *Turkish Rug House:* the company can now write down what its one agent exists to do and the most it may ever propose, approved by the person who holds Governance authority, changeable with the previous version kept intact — and the system says plainly that writing it down does not yet constrain anything. *Hebun AI:* one table, one writer, one read, one audit sibling, one enum value — ledger 39 → 40 — `agents` byte-unchanged, `permissions` and `role_permissions` still at zero writers, human-only CHECKs 8 → 9, and the second rung of the mandate ladder deliberately left `reached: false`.

## AMA-2 — the ceiling was five lines; finding out what it was allowed to compare took the day

- **The two vocabularies were never the same strings, and a fail-closed gate can enforce the wrong thing.** `mandate.proposal_scope` holds ALIASES (`"send"`); `prepared.actionKind` holds REGISTRY KINDS (`"send-external-communication"`). `agent-origination/contracts.ts` had said *"the alias never becomes the kind by string"* since AGENT-PROPOSAL-1, and the correspondence existed only as a fact about control flow — the inlet passes the constant because origination selected the alias. A ceiling comparing the two directly matches **nothing** and refuses **every** proposal, including the ones a mandate admits. **That defect is invisible to a test suite that only checks refusals: it looks perfectly fail-closed while enforcing the wrong thing.** The in-scope case is the assertion that catches it, and the bite-proof mutates the comparison and requires the SUCCESS case to break.
- **A prompt-shaping surface is not an enforcement seam.** AMA-1's closure had named `buildOriginationCandidates` as a candidate. It shapes what a model is offered, so a gate there is a **suggestion** — a proposal assembled another way never meets it. The enforceable seam is the one that makes the thing DURABLE, and there is exactly one for an agent. **Ask "what is the narrowest thing that must happen for this to become real?" — the gate belongs there, not where the decision looks like it is being made.**
- **A refusal NAME is not a READ, and a substring guard cannot tell them apart.** AMA-1's directory firewall banned the substring `agent-mandate` across the proposal path. The new refusal vocabulary legitimately contains it — `no-agent-mandate` and `action-outside-agent-mandate` are the *names of two refusals* — so the guard started failing on honest vocabulary. AMA-1 had already learned this exact shape on the writer that must SAY the words it denies, and I re-learned it on the same file one phase later. **Ban the mechanism (the import, the table, the read symbol), never the word.**
- **`indexOf("export function foo(")` is a guard that fails for the wrong reason the day `foo` becomes `async`.** `slice(-1)` is the file's last character, so two AGENT-PROPOSAL-1 assertions would have begun reporting a missing `AgentProposer` parameter that was plainly still there. Anchor on the NAME and assert the anchor was found. **A brittle anchor does not fail safe; it fails misleading.**
- **Three refusals, not two, and the third is the one the plan forgot.** AMA-1 predicted `no-mandate` and `mandate-authority-unavailable`. The out-of-scope case is a third organizational fact, and folding it into either would tell an operator to fix the wrong thing. **When a design predicts N failure states, count them again once the runtime exists — the one with no runtime yet is the one that gets folded away.**
- **A global zero is a weaker assertion than a delta, and a new precondition is how you find out.** Three released suites asserted `count(decision_records) = 0` — true only while nothing else wrote them. Bounding an agent is a real Governance act. The claim was never "this database holds no decisions", it was *"nothing was decided about this act"*. Rewritten as a delta across the proposal and as a scope over the action-authorization subjects, both are **strictly harder** to satisfy: a global zero also passes in a world where origination wrote a permit and something else deleted it. **A census that only holds while nothing else happens is measuring the fixture, not the claim.**
- **Six released suites breaking at their central assertion is the strongest evidence the gate is real.** AGENT-PROPOSAL-1, -2, -4B, SIA-1, SIA-2 and SIA-2.6 all went red the moment enforcement landed, because an agent that exists but is not bounded can no longer propose. **A constraint that breaks nothing when you add it probably is not one.**
- **Half the blast radius was invisible to targeted runs, and only the full suite found it.** The three I predicted exercise agent origination as their SUBJECT; the three I missed use it as a FIXTURE for something else — outcome observation, evaluation, invocation attribution. Grepping for the feature's own name finds the first group and not the second. **When you add a precondition to a primitive, the suites that break are the ones that use it as scenery, and you cannot enumerate those from the feature's name — run the full suite before believing the radius.**
- **Prove the outage case with the writer's own database healthy.** Turning off the whole control plane proves little — everything refuses. Renaming `agent_mandates` away leaves the proposal writer perfectly able to insert, so the gate is the **only** thing that stopped the row. **Simulate the failure of the authority you are testing, not the failure of everything.**
- **Import the READ SEAM MODULE, never the feature barrel.** `@/features/agent-mandate` re-exports `establishAgentMandate`; the barrel would have put a Governance-bound mandate WRITER into the proposal path's import graph for the sake of a read — G6C's defect, where a database-handle import dragged `establishGovernanceAuthority` into Heby's graph. And the ban has to be by **exact specifier**, because the legitimate read-seam path contains the barrel's own path as a substring.
- **Inverting a census is not relaxing it, if the exemption is a NAME.** AMA-1's *"exactly three, none a proposal path"* became *"exactly four, exactly one a proposal path"*. Exempting the seam's **directory** would have quietly let the decision writer, the permit consumer and the revoker acquire ceilings of their own. **Exempt the file, never the folder.**

**Weekly three.** *Learned:* the risky part of an enforcement phase is not the branch that refuses — it is proving the branch that ALLOWS still allows, and finding out which vocabulary the two sides were actually speaking. *Turkish Rug House:* the company's written-down statement of what its agent is for now actually stops that agent, and withdrawing it stops the agent completely — while the Director can still propose by hand anything the agent is refused, because a mandate bounds the machine and not the person. *Hebun AI:* zero schema, zero migration, ledger unchanged at 40, one enforcement seam proved unique by census, three fail-closed states that cannot collapse, 14/14 bite-proofs biting, six released suites that had to bound their agent before they could propose again — and the ladder's third rung deliberately still `reached: false`.

## AMA-3 — the class boundary was decided by a boolean, and the surface I expected to fix was already honest

- **`SourceResolution.authoritative` is ONE boolean per class, and that mechanical fact decided an architecture question I was about to answer by subject.** A mandate obviously "belongs with agents" — until you notice `agents` declares `authoritative: false` because it carries recomputed counts, and the class's own header says *"a class cannot assert one standing and cite under another"*. Folding a Governance-bound row a human wrote into a derived class would give the one thing on that surface a human actually DECIDED the standing of a recomputed number. **When two facts share a subject but not a STANDING, they are two classes — and the field that carries standing is where you look, not the subject.**
- **Not every phase that touches a surface owes that surface a correction.** The brief told me to fix the seeded catalog's presentation "as narrowly as necessary". Measuring first showed UI Phase 25B and AGENT-ID-0.1 had already done it: provenance badges, render order, an in-memory label in the page header. The correct output was a FIREWALL that pins what is already true and zero product change. **The instinct "we're in here anyway, let's tidy" is how a milestone's scope evaporates; measure the thing you were told to fix before you fix it.**
- **A ban on a WORD fails on the sentence that DENIES the word — third time in one program.** AMA-1 hit it on the writer that must say what it refuses; AMA-2 on a refusal NAME (`no-agent-mandate`); AMA-3 on an absence statement whose honest clause is *"refused before anything is written"* — the word `anything`. **The settled remedy is now a rule: ban the PHRASE that makes a claim, pin the denial by equality, and scope the guard to what the module asserts.** If a guard has failed this way three times, the guard shape is the defect, not the prose.
- **A directory ban and a file ban are different guarantees, and only one of them survives a product connection.** Two of AMA-2's directory-wide bans (`src/app`/`src/components`, `heby-answer`) had to open for AMA-3. Exempting the DIRECTORIES would have let any page in the product, or any file in Heby's answer path, quietly acquire a ceiling. Four named files, each separately proved unable to reach the proposal writer. **Exempt the file, never the folder — the second census inversion in two phases, and relaxed neither time.**
- **"Reads a mandate" and "enforces a mandate" were the same sentence only while nothing could DISPLAY one.** AMA-2's census asserted both at once and was right to. The moment a surface rendered a ceiling the two claims separated, and keeping them merged would have meant either weakening the enforcement claim or refusing the product. **When a census stops holding, check whether it was measuring two things under one name.**
- **A projection belongs to the authority that owns the facts, and the import must be the READ SEAM MODULE, not the barrel.** Third application of G6C: `@/features/agent-mandate` re-exports `establishAgentMandate`, so importing the barrel would have put a Governance-bound writer into Heby's graph for the sake of a read. Banned by EXACT specifier, because the legitimate relative import contains the barrel path as a substring.
- **Operator-authored prose goes in `content`, never `detail`.** E2-6's rule, and a mandate's purpose is exactly the case it was written for: `detail` flows into Heby's own validated response body, so a purpose sentence that happened to read like a claim would become a sentence Heby appeared to be making.
- **Verifying a product surface needed a real login, and the browser pane would not hold the session.** The honest path was: migrate the LOCAL dev database, provision a dev credential through the released hasher, seed an agent and two mandate revisions through the released writers, sign in, and read the page. Reverse-engineering Next's server-action wire format with curl was the wrong turn — two attempts, both landing on `error=invalid`. **When a harness fights you, use the product the way a human does before you start forging its protocol.**

- **A firewall saying "must not reach X" can become false because the PRODUCT legitimately grew, and the repair is to re-state what it was protecting.** AGENT-ID-0.1 asserted the `/agents` boundary must not reach `decision-authority`, and its own comment explained why Knowledge's boundary DOES: *"ratification is a governance act"*. Recording a mandate is a governance act in exactly the same way. So the boundary moved from *narrower than the released precedent* to *equal to it* — and the honest repair was not to delete the claim but to sharpen it: assert the reach EXISTS, and assert it runs ONLY through the mandate authority, by re-walking the import graph with that module cut out. **When an import-graph ban breaks, ask whether the thing it forbade is now reached THROUGH a legitimate owner or BESIDE one — those are different facts and only the second is a defect.**
- **The full suite found what targeted runs could not, again, and the shape was the same as AMA-2's.** Three failures, none in a file I had touched: two boundary censuses over the `/agents` action module and one `StateBlock` consumer census. Both are enumerations that exist precisely so a new capability cannot arrive silently — they did their job. **A census that breaks when you add something is the census working; the temptation to "bump the number" instead of naming the addition is the whole defect it was built to prevent.**

**Weekly three.** *Learned:* the two hard calls were both decided by facts already written in the repository — one boolean on a contract, and a page that had already been made honest two phases earlier — and neither would have been found by reasoning about the feature. *Turkish Rug House:* the Director can now open `/agents`, read in plain language what Heby exists to do and the most it may ever propose, revise that bound with the previous version kept intact, and ask Heby itself the same question — while every screen and every sentence says that being inside the bound is not permission to act. *Hebun AI:* zero schema, zero migration, ledger unchanged at 40, one new Heby source class (17th, `authoritative: true`), one product write path through the single released writer, the mandate census inverted a second time to eight named modules, and production acceptance deliberately left NO.

## AMA-4 — production acceptance found the defect that only production could find, and two of my own process errors invalidated results before it did

- **The acceptance proof was correct and unrecorded, which is not the same as proved.** Step D refused exactly as designed, and the durable record said `not-authorizable` — a value equally consistent with an unreachable authority, an unbounded agent, and an excluded kind. I established the cause by elimination: every other refusal ruled out by its own distinct reason, `prepareAction` computed to `REQUIRES_HUMAN_REVIEW` so the writer was provably reached, and the released AMA-2 suite re-run. That reasoning was sound and it is the wrong place for the answer to live. **If a milestone's central proof rests on inference over the production record rather than on a row in it, the instrumentation is part of the deliverable — fix it before closing, not after.**
- **A comment forbidding a collapse does not prevent one happening downstream of it.** `action-authorization/contracts.ts` says the three mandate states "MAY NEVER COLLAPSE", and they were collapsing two seams later: the inlet answers `not-authorizable` for every writer refusal its vocabulary cannot name, and origination stored the inlet's answer. Both files were individually honest. **A constraint stated in the module that OWNS a vocabulary is not enforced anywhere the vocabulary travels — trace the value to its last durable write, and pin it there.**
- **A source-text pin can freeze the defect it was written to prevent.** AGENT-PROPOSAL-4B pinned `filingRefusal: filed.reason` to prove no vocabulary was invented — and in doing so froze WHICH released vocabulary is recorded, which was the bug. The repair was to pin both operands, require the inlet's reason to remain the FALLBACK, and forbid a literal — a check the original never performed. **When a released pin blocks a real fix, ask whether it was asserting one claim or accidentally two; repair it stricter, never delete it.**
- **Carry the closed enum, not the prose that contains it.** `detail` already named the ceiling in a sentence — and for other refusals that same sentence embeds a recipient's display name. Storing it would have put operator-facing PII into a provenance column. **The reason is data; the sentence about the reason is presentation, and only one of them belongs in a durable column.**
- **A killed suite leaves mutated source on disk, and the next run silently tests the mutation.** A 10-minute foreground timeout SIGTERM'd a bite-proof mid-mutation, deleting an ownership gate from the agent retirement writer and turning a soft withdrawal into a hard `delete` in the Knowledge authority. I had already started a second suite against that tree. Second occurrence of this failure mode — the first was ENOSPC. **`git diff` against the released SHA before trusting any suite result and before every commit; the residue is in files you never opened.**
- **A suite run differently from how the repository runs it proves nothing.** I ran `node --env-file=.env.local scripts/run-tests.mjs`; the released path (`npm run test:run`) loads no env file. Sixteen gating-sensitive suites failed and I nearly reported them as regressions. **Run the repo's own command. If you must vary it, prove the variance is inert before you interpret a single failure.**
- **Read the summary line, not the tail.** The task notification reported exit 0 — that was my wrapper's trailing `tail`, not the suite, which had exited 1. And `tail -7` showed seven failures out of seventeen. **Two independent ways to under-read the same result; assert on the run's own summary and its own exit code.**
- **A structural constraint can force the acceptance ordering, and that is a finding, not an inconvenience.** `AGENT_ORIGINABLE_ACTION_KINDS` is `["send"]` — one kind — so `action-outside-agent-mandate` is reachable ONLY through an empty scope, because `[].some(...)` is false for everything. The second revision was therefore structurally required rather than manufactured for a test. Recording the empty ceiling FIRST also left production functional: the reverse order would have ended with the agent unable to propose anything, a capability regression wearing an acceptance's clothes. **Derive the order from what the vocabulary makes reachable, then check which end state you are leaving behind.**
- **REFUSED != COSTLESS.** Every out-of-mandate refusal still spent a real model call and wrote an invocation row, because the ceiling is enforced at the proposal writer — after the model has reasoned and selected. The mandate stops the durable proposal, not the thinking that produced it. **A ceiling that sits behind the expensive step bounds consequence, not cost, and saying so is part of describing it honestly.**
- **A wrong-workspace answer is indistinguishable from an absent truth, and that cost a round trip.** Step E first "failed" because the question was asked from the Heby panel on `/approvals`, which resolves to the `decisions` workspace — whose four classes do not include `agent-mandate`. Heby answered confidently from Governance and Knowledge instead. Every source it cited was honestly labelled, so nothing lied; but the reader cannot tell "not answerable here" from "no such thing". Same lesson E2-8 recorded, second occurrence. **Workspace availability is not Heby availability — verify the route resolves to the workspace that owns the class before calling a grounding gap a defect.**
- **Diagnose the grounding path before touching the prompt.** The instruction not to patch wording was the right constraint: the seam was healthy the whole time. Calling `readAgentMandateGroundingSource` directly against real data returned the effective revision, the superseded one, and the full ceiling semantics — which immediately moved the search upstream to routing, where the actual explanation was. **When grounding looks wrong, execute the grounding seam in isolation first; it either exonerates itself or names the defect.**

**Weekly three.** *Learned:* every genuine finding this milestone produced came from measuring production rather than from reasoning about the code — the collapsed refusal, the stale credential, the pg_dump mismatch, the wrong workspace — while two of my own errors came from running things differently than the repository does. *Turkish Rug House:* the company's written ceiling has now been recorded, enforced, revised and reported against the real system by a person clicking through the product, an agent proposal was refused for being outside it with nothing written anywhere, and a permitted one stopped at pending waiting for a human — nothing was sent. *Hebun AI:* production ledger 39 → 40, one defect found and repaired at the acceptance gate with zero schema and no new vocabulary, one released pin repaired stricter, `agents` byte-unchanged with `authority_ceiling` still NULL and no writer, permits and execution attempts both unmoved at 1, 618/618 — and the Agent Mandate program closed without closing its era or selecting a successor.

## Heby decision-queue grounding — the class name was right, the census was the reviewer, and the one-file proof was the weak spot

- **A declared-but-unconnected class is not automatically the right home — check whether the repository already defined it.** `decision-records` sounds like it means recorded Governance decisions, and it does not: its own contract says decision-PREPARATION material, both declaring workspaces also declare `decision-preparation`, and the `decisions` profile routes to `/approvals` saying "Heby prepares; it never approves". Three independent statements, all written before this phase. **The temptation was to rename or re-home it on the strength of the name; the repository had already answered, and reading it was cheaper than deciding.**
- **The riskiest import is the one that is closest.** Every previous grounding projection imported an authority from another directory. This one lives in the SAME directory as the decision writer, the proposal writer, the permit consumer and the permit revoker — so a single convenience import would have put a Governance-bound writer into Heby's graph. **When the writer is a sibling, a substring check on the import line is not enough: walk the value-import closure and assert the whole set, then prove the walker bites by running it against a known writer.**
- **A census that fires on your addition is the reviewer, and the repair is to name the addition — but check whether the census was as strong as it claimed.** APP-1's "no duplicate queue" pin caught the new module. Its stated property was fine and my module satisfied it. But its *proof* — "the one non-route importer holds no queue of its own" — was hardcoded to a single file path, so the permitted list could grow while only the original entry was ever measured. Adding my file to the list and nothing else would have been the bump-the-number move. **Generalising the loop to every non-route consumer made the list admit by measurement rather than by trust; a census whose proof does not scale with its list is a census that quietly stops working.**
- **Do not compute a truth another class already owns.** "How long has it been waiting?" was explicitly requested, and E2-4 already owns duration over this same seam. Carrying the stored timestamp and refusing to derive elapsed time kept the class `authoritative: true` and avoided two modules measuring one interval against two clocks. **The second implementation of a derived fact is how two surfaces come to disagree about the same row.**
- **UNAVAILABLE != EMPTY QUEUE is the most expensive sentence this class could get wrong.** Every other collapsed-state defect in this repository misled a reader; this one would make a Director act by doing nothing during an outage. It is pinned in both directions — the unavailable path carries the authority's reason verbatim, and is asserted never to emit the empty-queue sentence in those words or any others.
- **A pre-existing module-init cycle in `src/db/schema/` only appears when a test imports a schema file directly.** The barrel resolves the order; a direct submodule import throws `Cannot access 'tenantColumns' before initialization`. The app never hits it because `db/client.server` loads the barrel. **Reproduce the app's import order in the test and say why, rather than restructuring product code to satisfy a harness** — but the cycle is real and worth repairing on its own terms one day.

**Weekly three.** *Learned:* the two decisions that mattered were both already written down — what the class means, and what the census was really asserting — and the only new judgement was noticing that a released proof did not scale with the list it guarded. *Turkish Rug House:* the Director can now ask Heby what is waiting for their decision and get the recorded consequences, evidence state and proposer back, while the deciding stays exactly where it was, on the approvals surface. *Hebun AI:* zero schema, zero migration, ledger unchanged at 40, no new authority, one released read seam reused with no second reader, an import-closure firewall that self-tests, one released census repaired stricter, and 620/620.

## Heby decision-queue grounding, production acceptance — the restraint was the thing that worked, and the one open question answered itself in the evidence

- **Refusing to compute a duration is what made the duration correct.** Heby said the oldest proposal had waited "approximately 3 days and 22 hours", which looked at first like the model inventing arithmetic from `filedAt` timestamps — the exact failure the single-owner rule exists to prevent. The turn's own evidence settled it: E2-4's `operations` item was in the same context reading `3 awaiting · oldest waiting 3d 22h · basis action-request.created_at`, and Heby restated it. **Because this class carried no duration, exactly one number reached the model. Had it carried its own, two would have — and the model would have picked one, silently.** The design's payoff was invisible until a human asked the question that could have exposed it.
- **Provenance questions are answered from the persisted turn, not from the code.** "Did the model calculate that?" is unanswerable by reading the module. It took one query against `heby_answer_source_evidence` for that message id, and the answer was verbatim. **A grounding system that persists what it grounded on can settle its own truth-semantics disputes; one that does not can only argue about them.**
- **`recordRef` is evidence identity, and tidying it costs more than the noise it removes.** Heby printed raw proposal UUIDs, which are not displayed anywhere on `/approvals` — measured, not assumed — so they are unmatched noise for the human. The available "fixes" were both worse than the problem: weakening `recordRef` breaks the property that the model can never invent a reference, and telling the model not to print it is prompt-shaping. **Declining a correction the Director explicitly left open, with the reason stated, is part of the job — target, kind and filed time already identify each item to a human.**
- **The acceptance question to ask is the one whose wrong answer is expensive.** Not "did it list them" but: did it exclude the approved one from pending truth, did it advocate, and where did every number come from. All three were checkable against production rows, and the third is the only one that would have been missed by reading the answer alone.

**Weekly three.** *Learned:* a restraint written into a module months of phases apart from the question it answers only proves itself when someone asks — and the proof lived in persisted evidence, not in the code. *Turkish Rug House:* the Director can now ask what is waiting and get three real proposals back with their consequences and how long the oldest has sat, while the approved one is correctly left out and nothing in the answer pushes toward approving any of them. *Hebun AI:* production-accepted with zero schema, zero migration, zero new authority, zero production mutation, one released census repaired stricter, one cosmetic limitation recorded rather than papered over, and 620/620.

## Human Legibility Reach — the discovery's first answer was wrong, and being wrong is what made the milestone small

- **A `from(users)` grep is not a census of who reads `users`.** The discovery concluded "Hebun cannot name a human" and named it the strongest bottleneck. Then a second pass on raw SQL found `readDelegationCandidates` already enumerating members by `coalesce(display_name, name, email)`, released, tenant-scoped and authority-gated. **The authority question had been answered and shipped; only reach was missing.** The correction turned a proposed authority into a read seam and a picker. Grep for the ORM call *and* the table name in raw SQL, or the census is fiction.
- **Reusing a released query's semantics is where the bug hides, not where the safety is.** `readDelegationCandidates` excludes the caller because self-delegation is invalid. Department ownership is the opposite — OSA-2 recorded the tenant's only human as owner of Engineering — so inheriting that one predicate would have returned an **empty picker for the only organization that exists**, which reads as a broken feature rather than a wrong rule. **Copy the shape, re-derive every predicate.**
- **A firewall assertion can be satisfied by a DECLARATION.** `code.includes("LABEL_UNAVAILABLE")` passed while the rendered constant had been replaced by an invented `"Unknown"` — the bite-proof survived and exposed it. OSA-2's `A FIELD THAT IS POPULATED != A FIELD THAT IS RENDERED`, in a new costume. **Assert the rendering position, not the constant's existence.**
- **A bite-proof can survive because a sibling predicate covers it.** Removing `isNull(memberships.revokedAt)` changed nothing, because every revoked fixture also carried `status = 'revoked'`. The guard looked proved and was not. It took a row whose **two revocation facts disagree** to make the predicate independently load-bearing. **Defence in depth makes each layer untestable unless a fixture isolates it.**
- **`owner_actor_id::text ~ '[A-Za-z ]'` is a broken "no name was persisted" probe — a uuid's hex digits are letters.** It failed against correct code. The honest probe casts the whole row to text and scans for the actual label strings, which also catches a label written into a column a later phase adds.
- **The gate measured someone else's rule and left it alone.** `setDepartmentOwner` checks `memberships.lifecycle_status` and nothing else, so the released writer accepts a revoked member and a soft-deleted identity as accountable. An assertion expecting a refusal got `recorded` — the test was wrong, not the code. **Not repaired: widening or narrowing another authority's rule is not a legibility milestone's to do.** What was owed was the direction, and only one is safe: `THE PICKER OFFERS FEWER PEOPLE THAN THE WRITER ACCEPTS. NEVER MORE.`
- **A blocked verification is reported, not routed around.** The read-only production check was declined by the permission layer. The script exists and is unrun, production acceptance is recorded as PARTIAL at four of seven items, and the local disposable-database render is labelled as a proof of the mechanism rather than a measurement of production.

**Weekly three.** *Learned:* the most valuable thing in the turn was discovering that the previous turn's headline finding was false — the capability existed, gated correctly, with one consumer — which replaced a proposed new authority with a read seam and a `<select>`. *Turkish Rug House:* a Director naming who is accountable for a department now picks a person from a list of names instead of pasting a UUID, and the department says who owns it readably while the record still holds the identifier. *Hebun AI:* zero schema, zero migration, ledger unchanged at 41, no new authority, no roster, Heby deliberately untouched, one released pin repaired stricter with a new bite-proof, one standing observation recorded about another authority's rule, 9 of 9 bite-proofs bit, and 625/625.

## OSA owner eligibility hardening — the UI was doing the enforcing, and two guards were untested

- **A control that hides somebody is not enforcement.** `setDepartmentOwner` checked `memberships.lifecycle_status` alone while the owner picker required an unrevoked membership and a live identity, so the product looked correct and the authority was not: any caller with a session could set any uuid. The fix was not new logic — it was making the writer consult the rule the picker already used. **When a surface is stricter than the authority behind it, the surface is the security control, and surfaces are not security controls.**
- **The semantics already existed; the work was finding which module owned them.** The membership half is the SESSION predicate — `identity-repository.server.ts` applies it at three sites to decide whether a human may hold a session at all — and the identity half is the login check plus the soft-delete column the durable-identity writer already used. Nothing was invented, and a firewall now reads the session path so a change there is visible here.
- **A shared rule can be a predicate rather than an authority, and the file extension decides.** OSA's released firewall pins the writer's reachable `.server.ts` modules to an exact list. Shipping the rule as a server module would have forced that list open; shipping it pure — drizzle conditions, no handle, no query — meant the pinned assertion passed untouched. **Purity is not a style preference when a firewall measures reachability by module kind.**
- **Two conditions checked separately need two fixtures, each isolating one.** Removing `memberships.status` changed nothing because every revoked fixture also carried `revoked_at`; the sibling caught it and the bite-proof survived. This is the same failure as the previous milestone, from the mirror side — and the previous fix did not prevent it, because only one of the two mirrors existed. Defence in depth makes each layer untestable until a fixture defeats every other layer.
- **A mutation that does not compile is testing the module loader.** A bite-proof re-wired the writer to a helper the writer does not import; the `ReferenceError` killed the suite at its first assertion and the proof "bit" for a reason unrelated to the guard. Inline what the mutated file cannot resolve.
- **Moving a condition breaks every proof anchored to where it used to live.** The first final suite came back 627/1: four legibility bite-proofs could not find their strings because those conditions had moved into the shared rule. **The harness refusing to run them is the design working** — `withMutation` asserts the find-string exists before mutating, precisely because a mutation that cannot be applied looks exactly like one that failed to bite. Without that check four guards would have reported as proved while testing nothing.
- **A monotonic change can be accepted in production without re-observing it, if you say which condition the argument does not cover.** The hardening only ADDS conditions, and the real human demonstrably passes five of the six because they signed in. The sixth is named as unproven rather than waved through, and the unobserved render is recorded as open rather than folded into a pass.

**Weekly three.** *Learned:* the defect was found by a test that expected a refusal and got a success — a disagreement between two of our own components, not a report from outside — and two of our existing guards turned out to be untested for structurally identical reasons. *Turkish Rug House:* a human whose membership was revoked or whose account was deleted can no longer be recorded as accountable for a department, whoever calls, and a department whose owner later leaves keeps naming them instead of being quietly rewritten. *Hebun AI:* zero schema, zero migration, ledger 41, no new authority, one rule stated once for three consumers, two released pins repaired stricter, 7 new bite-proofs and 9 re-anchored ones all biting, and 628/628.
## WORK-1 — the missing primitive was a table nobody had written, not a table nobody had activated

- **The deciding argument for reuse was absent, and that is what made a new table correct.** OSA-0 activated `departments` because `agents.department_id` was a LIVE foreign key a rival table would have orphaned. For work, every inbound reference to `tasks`/`goals`/`plans`/`missions`/`workflows` comes only from other dead tables — measured, not assumed — so authoring a new one orphans nothing. **A precedent is a piece of reasoning, not a rule; check whether its premise still holds before you inherit its conclusion.**
- **Activating `tasks` would have shipped a second execution ledger, not a work register.** `task_status` is `pending·running·failed`, `task_execution_type` includes `scheduled` and `event-driven`, and `executions.task_id` points at it. The danger was never the extra columns — it was that one live FK would have made `executions` addressable beside `action_execution_attempts`, the one table Era III pins to a single reader.
- **The five-mutation shape was already written; the work was refusing to add a sixth.** Gate → transaction → `FOR UPDATE` → mutate → audit in the same transaction → typed refusal is OSA-1's, verbatim, and the eligibility predicate is Identity's shared one imported unchanged. **Nothing about a new authority is new except what it owns**, and every place this milestone re-derived something instead of importing it would have been a place two answers could disagree.
- **A firewall assertion with an `|| true` in it was written, noticed, and deleted.** It made "the accountable actor type is a literal" unfailable. The replacement enumerates every assignment to that field and was **bite-proofed**: rewiring the writer to take the type from input made it fail with the offending expression quoted. *An assertion that cannot fail is not a test* — and the only way to know which one you wrote is to break the code on purpose.
- **A substring ban trips on somebody else's vocabulary.** `no governance_domain value may name work` failed against `workflow`, a value from the foundation baseline. The honest form is an exact-value ban on the names this milestone could have added. **A test that fails against code you never touched is measuring the wrong thing, however good its intention.**
- **A source regex cannot see across a string concatenation.** The surface's "Hebun did not observe it, did not verify it" is split over two lines by the formatter, so the assertion failed against correct code. Asserting the exported **constant** as well means a reflowed line cannot silently lose the sentence.
- **Shipping an authority falsifies sentences written when it did not exist, and one of them was ours.** `live-map/contracts.ts` said the `agent -> work` edge is absent because there is "no authority". There is one now, and the edge is still absent — because a database CHECK forbids an agent from being accountable. The repaired reason is **stronger** than the one it replaced. The stale sibling line above it (`agent -> department  no department authority exists`) has been false since OSA-1 and was **deliberately left**, recorded rather than fixed: repairing another milestone's residue is not this one's to do.
- **A suite that ran against a moving tree is not a measurement.** A comment repair landed while the final run was in flight. It was killed and re-run clean rather than reported. The number is only worth what the tree was doing while it was produced.
- **Adding one authority cost 49 test-file edits, and not one failure was a defect.** The first complete suite was 596/34/630. Absolute ledger pins, migration allowlists, the audit-sink owner census, the human-only CHECK census (ten → eleven), the server-action boundary census, the dashboard route count, the `StateBlock` consumer census, two consumer censuses, five release-digest constants and one pending-migration probe — every one legitimately grown by WORK-1 doing exactly what it was authorized to do. **Every one still asserts exactly what it asserted**, as an exact list or exact count, so the repair was mechanical rather than a weakening. G6D already made fifteen ledger pins phase-relative for this reason and **the class regenerated**. Budget ~50 test edits for the next authority that authors a migration.
- **A bare token is a substring of somebody else's name.** `k1-flow` forbade any migration matching `/k1/` and therefore failed against `wor`**`k1`**`_organizational_work_authority`. The same shape as the `governance_domain` substring ban tripping on `workflow`, twice in one milestone. **Delimit the token or you are matching prose, not names.**
- **An index is not "the last one".** `CANONICAL[40]` meant "the newest migration" and has needed two numbers moved in lockstep at every release since it was written. `.at(-1)` is what it always meant. Both this and the `/k1/` bug were latent and pre-existing — WORK-1 only made them fire.
- **A probe must follow what the PENDING migration adds, and "pending" moves every release.** `canonical-prefix-postgres.ts` probed `departments_no_second_parent_chk` because OSA-1 created no table. WORK-1 does create one, so the probe went back to being a table probe. The rule in that file's own comment is the durable part; the constraint name in it is not.
- **A truncated capture of a passing-looking run is worse than no run.** The first clean suite was captured with `| tail -12`, which returned exactly twelve `FAILED` lines and **no count** — so the summary and any earlier failures were cut off, and the pipeline's exit code was `tail`'s, not the suite's. `0` meant nothing. **Never pipe a verdict through `tail`.**
- **A membership cannot be moved between tenants, and the fixture that tried was the wrong fixture.** `memberships` carries a composite `(tenant_id, role_id)` FK. Seeding a genuinely second member was not a workaround — it was what the section needed, because proving five ways of becoming ineligible requires first proving the member was eligible.

**Weekly three.** *Learned:* the gate's hardest question was not "which table" but "whose argument am I borrowing" — OSA-0's reuse decision rested on a live FK that does not exist here, and checking that premise is what turned a tempting activation into a clean additive table. *Turkish Rug House:* the Director can now record that a real piece of work exists, file it under a department, name the accountable person from a list, and declare whether it is planned, active, blocked or complete — with the product saying in words that "complete" is a declaration and not a verified outcome. *Hebun AI:* one new authority, one additive migration with no DROP and no INSERT, ledger 41→42 locally and 41 in production, zero Governance decisions, zero agent authority, zero Heby change, one released comment repaired stricter, one stale sibling recorded rather than fixed, one no-op assertion caught and bite-proofed, 49 test files' released pins and censuses grown without weakening one of them, two latent test bugs repaired, and 630/630.


## WORK-1 production acceptance — the ceremony that did less than planned, on purpose

- **A non-effect claim is stronger measured by the WINDOW than by a delta.** A before/after count needs a baseline captured at exactly the right instant, and I did not have one. Asking each table how many rows it gained since a timestamp comfortably before the act needs no baseline at all — and asking the whole `audit_log` what it recorded in that window collapses twenty separate claims into one line: **the only event in the window was `work_item / work.recorded`.** Prefer the question that does not depend on remembering to look first.
- **`version = 1` and `created_at = updated_at` are the proof that nothing touched the row afterwards.** Two columns the base contract gives every table for free, and together they answer "was this written once?" without reading any history.
- **The Director declined the scripted state transition, and that was correct.** The plan said move the item to `blocked` and back. Real work that is genuinely active is not blocked, and **falsifying organizational truth to complete a ceremony is worse than an incomplete ceremony**. Recorded as NOT EXECUTED IN PRODUCTION with the exact cost stated: the mutation is proved by released tests against a real database and proved nowhere in production, and it closes for free the first time this work legitimately changes state.
- **No safe subject means no test, not a manufactured subject.** Production holds one user, one membership and zero already-ineligible members. Proving the ineligible-human refusal would have meant revoking or soft-deleting the organization's only human identity. Refused, and recorded as PARTIAL. **An acceptance that damages real state to reach PASS is not an acceptance.**
- **The `pg_dump` version gate is three-for-three.** 14.20 on PATH against an 18.6 server, refused before the confirmation prompt, at AMA-4, OSA-2 and now WORK-1 — every time because `postgresql@18` is installed and simply not first on `PATH`. A gate that fires this reliably on the same cause is telling us the environment is wrong, not the gate.
- **Verify the migration ran; do not infer it from being told.** The ceremony ran outside this session. Schema truth was re-derived independently — prefix verdict, per-file digest, every constraint's `convalidated`, the enum's exact labels and order, the base-table delta — because "the ceremony reported success" and "the schema is what the release says" are different claims.

**Weekly three.** *Learned:* the strongest evidence in the whole ceremony came from asking a question that needed no baseline — one window query over `audit_log` proved twenty non-effect claims at once. *Turkish Rug House:* the company now holds one real, durable record that "Hebun Era III development" exists, sits under Engineering, and has a named accountable human — visible to the Director and grounded in the same authority chain as every other organizational fact. *Hebun AI:* production ledger 41 → 42, checkout and production converged, one authoritative row, one audit event, zero effects anywhere else, two production evidence gaps recorded honestly rather than manufactured, and no runtime code changed during acceptance.

## WORK-2 — grounding is cheap when it authors no migration, and one released boundary had to be restated out loud

- **The pin debt is MIGRATION-driven, not change-driven.** WORK-1 authored one migration and cost 49 test-file edits. WORK-2 touched five source files, added a source class, changed two authorities' contracts and a workspace profile — and cost **four**. Nothing counts the ledger when the ledger does not move. That reframes the debt: it is not "every change is expensive", it is "every migration is expensive", which is a much narrower thing to fix later.
- **A `never` exhaustiveness guard is worth more than any test here.** Adding `work` to the source-class union broke `source-resolver.ts` at typecheck with `Type '"work"' is not assignable to type 'never'` — before a single test ran. The pure resolver could not silently omit the new class. **An exhaustiveness guard on a closed union is a firewall that costs one line and never goes stale.**
- **"Connected" has three ways to be a lie, and the test must refuse all three.** A declared class with no substitution; a substitution no workspace reaches; a context item that never reaches the answer. The only proof that closes all three is driving the REAL answer flow and reading the composed `ModelGenerationRequest` — asserting on the projection's return value proves the first hop and nothing else.
- **When a milestone falsifies a released boundary claim, restate it — do not patch the filter that caught you.** HLR asserted "the whole Heby tree is blind to this module": no human's readable name reached Heby's context. WORK-2 breaks that on purpose. The lazy fix was to correct HLR's `file.includes("heby")` path heuristic, which was matching my file by its NAME. The honest fix was to notice the heuristic was catching something real and repoint the claim: the Heby subsystem still holds no legibility read, and EXACTLY ONE authority-owned projection resolves labels — named in the assertion, so a second one still fails. **A test that trips for the wrong mechanical reason can still be telling the truth about your design.**
- **Human-authored text goes in `label` if it is a NAME and `content` if it is a SENTENCE.** `work-artifacts` puts its title in `label`; AMA-3 keeps its mandate `purpose` in `content` so it cannot be read as a claim Heby is making. `label` and `detail` both flow into Heby's own deterministic prose; `content` reaches only the model. A work title is a name, so it follows `work-artifacts`.
- **`authoritative: true` says WHOSE RECORD IT IS, never that the world matches it.** The class declares true because `work_items` IS the record — and the provenance carries the second half in the sentence a model actually reads: every state is DECLARED, Hebun observed nothing and verified nothing. Those two facts are not in tension, and separating them is what lets a record be authoritative and unverified at once.

**Weekly three.** *Learned:* the milestone's real work was not the projection, it was noticing that HLR's firewall was correctly objecting to a boundary WORK-2 was crossing on purpose — and restating that boundary instead of silencing it. *Turkish Rug House:* the Director can now ask Heby what the company is working on and get the real recorded work back, with the accountable person named readably, the department, and the state a human declared — and Heby saying plainly that it did not watch any of it happen. *Hebun AI:* zero schema, zero migration, ledger unchanged at 42, no new authority, no write authority reachable from Heby, one source class added and scoped to one workspace, one released boundary repointed with its guarantee intact, four pins moved, and 632/632.

## WORK-2 production acceptance — the model repeated the semantics because they were facts, not instructions

- **Carrying truth semantics as DATA survives a live model; prompt prose would not have.** Unasked, Heby said "Hebun did not observe or verify the work", "not an observed or verified one", and "the declaration is mutable". None of that is in a system instruction — it is the class provenance, the per-item non-claim and each state's meaning string, all sitting in the grounding context as facts in front of the model rather than rules behind it. **A model forgets an instruction and cannot forget a field.**
- **The sharpest test was the negative one, and it passed twice over.** Asked what work is blocked, Heby said the context carries none, named the only item as active, and added "I cannot invent blocked work items that were not provided in the grounding context" — then did NOT generalize it into "nothing in the company is blocked". Corroborated: zero in-service work declared blocked. **A grounding system is worth what its absences are worth.**
- **The label resolved to an EMAIL, and that is the finding production had to reveal.** `coalesce(display_name, name, email)` is HLR's released expression, and the identity has neither name column set — so the "readable name" WORK-2 puts into a model's context is an address. Not a WORK-2 defect and not a resolution failure: gated, tenant-scoped, eligible, identifier preserved beside it. But the consequence IS new — that value now leaves the process to a third-party provider on every Command answer, where before it appeared only in two server-rendered pickers. **Recorded as a Director decision rather than closed, because choosing what a person is called is not a grounding milestone's call.**
- **A pre-probe baseline is worth capturing even when you expect nothing to move.** Three live model answers, zero drift across nine tables, zero audit rows in the window, `version` still 1 and `updated_at` untouched. The claim "grounding mutates nothing" is only worth what the measurement before it was worth.
- **IMPLEMENTED, CONNECTED and PRODUCTION-VERIFIED are three different words and the closure names all three.** The class existed at implementation; it reached the model request at release; a human asked a live provider and the answer was corroborated against the row at acceptance. Collapsing any two of those is how a milestone gets called done early.

**Weekly three.** *Learned:* the semantics held under a real model because they travelled as data — and the one thing production told us that no test could was what the "readable name" actually resolves to. *Turkish Rug House:* the Director can now ask Heby what the company is working on, who is accountable and what is blocked, and get answers that are true, bounded, and honest about what Hebun did not watch. *Hebun AI:* zero schema, zero migration, ledger 42 converged, three probes corroborated fact by fact, zero production mutation, one privacy consequence surfaced rather than buried, and 632/632.

## WORK-2 privacy hardening — the acceptance found a fact about production, not a bug in the code

- **A production acceptance can surface a consequence no test could have.** Every WORK-2 assertion was about the code, and the code was right: gated, tenant-scoped, fail-closed, identifier preserved. What acceptance revealed was a fact about the *row* — the production identity has neither name column — and therefore what the released `coalesce(display_name, name, email)` actually resolves to for the one human that exists. **A green suite proves the mechanism; only production tells you what the mechanism is handed.**
- **The same value can be correct in one audience and a disclosure in another.** Nothing about the string changed at WORK-2. What changed was that it started being composed into a request bound for a third party. `UI LEGIBILITY != MODEL PROVIDER DISCLOSURE`, and `AUTHORIZED TO READ != NECESSARY TO DISCLOSE` — being entitled to read a field internally is not a reason to send it out.
- **The cheapest correct boundary was a second projected EXPRESSION, not a second module or a filter.** One private resolver, two exported reads, one gate, one predicate, one query, one bound — and the only difference is which columns are selected. A filter after the fact, or a second module, would have created two places for the gate to drift.
- **Field provenance beats every string heuristic, and the failure is in both directions.** An `@` test erases a human legitimately named `someone@example.com` and waves through an address stored in `name`. Only the column that produced the value is honest, and only Identity holds that. Both modules are asserted to contain no `@` test, no regex, no local-part split.
- **ABSENT and PRESENT-BUT-EMPTY are different facts, and the caller renders them differently.** Making the expression nullable was half the change; skipping null rows so the id never enters the map was the other half. D2 bit exactly there — without the skip, a nameless human is present with a null value, indistinguishable to a caller from somebody Hebun can name.
- **A pin that moves from 1 to 2 must be pinned at 2, plus the relation between them.** HLR's "the label expression is written once" became "exactly two expressions, each written once, and the name expression is the label expression minus the address floor". That is stricter than what it replaced — a third fails, either being duplicated fails, and the two can no longer disagree about precedence.
- **The expectation in a bite-proof must name the assertion that ACTUALLY fires.** Two of six were wrong on the first run: D5 trips an earlier identifier assertion than the one it was written against, and D6's import alias `resolveHumanNames as resolveHumanLabels` keeps the local name, so the *adoption* check catches it and the *usage* check passes. Both "failed for the wrong reason" — which is the harness doing its job. **A bite-proof whose expectation you never verified is a test you have not read.**
- **Prove absence over the WHOLE composed request, not the field you expect.** The evidence array is where the label was put; a disclosure arriving through the instruction, the question or the history would be exactly as much of a disclosure. Serializing the entire `ModelGenerationRequest` and asserting no `@` anywhere costs one line and stops measuring only the path you imagined.
- **Scope by walking the real graph, not by grepping filenames.** 547 modules transitively from the answer path, searched for executable code naming an email: exactly one reads a human's address as a label. `TenantContext` carries none. Two other modules floor at an address and are outside the graph — recorded as untouched with the reason, rather than swept in.

**Weekly three.** *Learned:* the strongest evidence for a privacy claim was the negative one measured in the right place — real SQL for "an email-only human resolves to nothing", the whole composed request for "no address reaches the provider". *Turkish Rug House:* when the Director asks Heby who is accountable for a piece of work, the company's own email address no longer travels to a third-party model provider; the answer says `name unavailable` and keeps the authoritative reference, and the two internal pages still read normally. *Hebun AI:* zero schema, zero migration, ledger 42, no new authority, one new Identity projection sharing one gate with its sibling, one released firewall narrowed rather than weakened, one census pin moved from 1 to 2 with the relation pinned, 6 bite-proofs all biting, and 634/634.

## WORK-2 privacy hardening — production acceptance, proved by the request rather than the answer

- **The acceptance criterion was the REQUEST, not the RESPONSE, and that made it free.** "Email absent from the model's answer" is a weaker claim that would have cost a real billable disclosure to a third party to observe. "Email absent from the provider-bound grounding" is the claim that matters, and it is measured by capturing the `generate` seam against production data — zero provider calls, zero rows written. **When the honest question is about what you SEND, you never have to send it.**
- **Assert absence over the WHOLE serialized request.** The label lives in the evidence array, but a disclosure arriving through the instruction, the question or the history is exactly as much of a disclosure. Three booleans over the entire request — exact address, any `@`, local-part in any casing — cost one line each and stop the proof from measuring only the path I imagined.
- **The condition was verified live, not assumed.** `display_name ABSENT / name ABSENT` was read from production before the probe. Without that, "the label reads `name unavailable`" would have proved nothing about the case the hardening exists for.
- **A derived table list cannot forget a table.** Non-effects were measured by asking the catalogue for every `public` table with a `created_at` — 58 of them — and then asking each what it gained in the window. A hand-written list had already failed twice in this session (`action_authorizations` does not exist, `audit_log.event_type` does not exist). **Deriving the list turned two errors into zero and made the claim stronger than the enumeration it replaced.**
- **A probe against production must be built to write nothing, not merely intended to.** `getConversationRepo: () => null` was passed deliberately: the answer path persists an exchange when a repo is available, so the default would have written rows in production to prove that nothing writes rows in production.
- **The live product probe was NOT run, and that is recorded rather than blurred.** It needs an authenticated Director session. The criterion was met by the seam; the prose answer would have added a weaker observation at real cost. **Naming what you did not do is part of the acceptance, not an apology for it.**
- **`.env` values can be quoted, and a regex that forgets it produces a hostname called `base`.** One minute lost to `getaddrinfo ENOTFOUND base` because the captured value still carried its surrounding quotes. Strip them at the seam that reads the file.

**Weekly three.** *Learned:* the strongest acceptance evidence cost nothing to gather, because the claim was about what leaves the process rather than about what comes back — and a catalogue-derived table list beat the hand-written one that had already been wrong twice. *Turkish Rug House:* the company's own email address no longer travels to a third-party model provider when the Director asks Heby who is accountable for a piece of work; the answer names the work, keeps the authoritative reference, and says `name unavailable` instead of guessing. *Hebun AI:* deployed commit identical to the release commit, ledger 42 → 42, 58 tables scanned and none grew, zero audit rows, zero provider calls, zero mutations, and the one thing not executed named in the closure rather than folded into a pass.

## Departmental Placement — the firewall told me the schema shape, and the review caught what the tests would not have

- **A released firewall can be a design input, not an obstacle.** The obvious shape was `memberships.department_id`. `write-structure.server.ts` states — and asserts over its import graph — that it "never writes `memberships`", the row a session reads to build a `TenantContext`. Honouring that is why placement became its own table with its own blast radius, and why the structural writer is byte-untouched. **When a guard objects to your first design, read what it is protecting before you widen it.**
- **Check whose argument you are borrowing, again.** WORK-1 chose a new table because reuse would orphan nothing; OSA-0 activated `departments` because a live FK existed. Here the deciding fact was a third one — a SECURITY boundary — and neither precedent applied. Three milestones, three different reasons, same question asked each time.
- **The parent-side unique index existed for exactly this.** `departments_tenant_id_uq`'s own header says it exists so other tables can carry `(tenant_id, department_id)` and have PostgreSQL enforce tenant safety. Using it meant cross-tenant placement is UNREPRESENTABLE rather than refused — proved by asking the database to accept the row the writer rejects and reading `23503`.
- **Designing the disclosure boundary before writing code cost nothing.** WORK-2 spent a whole milestone discovering an email reaching a provider. Here the projection consumed `resolveHumanNames` from its first line, and the bite-proof that reintroduces the product label is a one-line mutation. **The expensive version of a lesson is the first time; the cheap version is every time after, if you write it into the Definition of Done.**
- **A new source class was justified by the CLASS CHARTER, not by the authority owner.** Same owner as `organization`, and still a new class — because that class carries "EXACTLY ONE ITEM, ALWAYS", says "no member roster is carried", and keeps its owner as an IDENTIFIER. Folding placements in would have falsified three released sentences to save one entry in a census.
- **Pre-closure review caught two defects a green suite would have shipped.** The read hand-wrote the six eligibility conditions as raw SQL — correct that day, and the exact drift the shared rule exists to prevent. And a structural probe was measuring the unique index instead of the foreign key, because one defence fired before the one under test. Neither was a test failure; both were found by reading against the Definition of Done.
- **A bite-proof found a third, in my own test.** The FK probe borrowed a human another section had placed, so mutating withdrawal changed which constraint fired. **A probe that depends on another section's outcome is not a probe** — it now uses a fixture the test never places.
- **The word-ban-on-honest-prose trap fired three times in one loop, twice in code I had just written.** `/verified/` against "verified nothing"; `/works in/` against "which department a human works in has its own authority". The fix is always the same and always available: ban the AFFIRMATION, not the word, and assert the denial is present so the ban cannot pass by silence.
- **A text search is not an import graph, and it answered wrong three times in one file.** `"human-department-placement"` contains the module's filename; `import type` erases; naming a module in a string imports nothing. Every time, the honest fix was to resolve value edges instead of matching prose.
- **Migration pin debt is ~36 files and entirely mechanical, and one pin is not.** `prodmig`'s probe must follow what the PENDING migration adds — `work_items` had become an EARLIER migration's table, so probing it would have been satisfied before migrating and proven nothing. That comment predicted its own next failure and was right.

**Weekly three.** *Learned:* the repository's own released firewalls chose the schema shape, and the pre-closure review — not the suite — found the two defects that mattered. *Turkish Rug House:* the Director can now record which department each person works in, see it on the Organization page, and ask Heby "who works in Engineering?" — with the answer saying plainly that a placement is a declaration, not an observation, and not a role, a reporting line or an authority. *Hebun AI:* one additive table, ledger 42→43, one new writer inside an existing authority, no new Governance domain, no action kind, no execution, the 19th source class scoped to Command only, provider-safe names by design, 8 bite-proofs all biting, thirteen released claims repaired rather than left stale, and 638/638.

## Departmental Placement production acceptance — two disclosure policies, same row, same instant

- **The strongest evidence in the whole loop was a contradiction that is not one.** The Director's screen said `senoltr@gmail.com works in Engineering`; the provider-bound context for that same row said `name unavailable (d5b496df-…)`. Same human, same placement, same moment. That is `UI LEGIBILITY != MODEL PROVIDER DISCLOSURE` demonstrated rather than asserted — and it only became demonstrable because the previous loop separated the two reads at the column level instead of by inspecting strings.
- **Designing the disclosure boundary into the Definition of Done cost one import.** WORK-2 spent a milestone discovering an address reaching a provider. Here the projection consumed `resolveHumanNames` from its first line, and production confirmed it on the first attempt. **The expensive version of a lesson is the first time.**
- **One window query over `audit_log` proved every non-effect at once, again.** Exactly one event type in the window: `department_placement / organization.placement.set`. No baseline needed, nothing to remember to capture beforehand.
- **A catalogue-derived table list caught something a hand-written one would have hidden — and it was innocent.** 59 tables scanned, two grew: the placement, and `user_session_contexts`. The second is the Director's own sign-in to perform the acceptance. **Name it and explain it rather than filtering it out**; a non-effects claim that quietly excludes what it cannot explain is not a measurement.
- **`version = 1` plus `created_at = updated_at` is still the cheapest write-once proof there is.** Two columns the base contract gives every table for free, and together they close "was this touched afterwards?" without reading any history.
- **The refusals were proved where they can be proved.** Production holds one human and one department, so exercising revoked-member or retired-department would have meant damaging real state. Proved against a real database in the released suite and classified as such — an acceptance that damages real state to reach PASS is not an acceptance.
- **`memberships` was verified unwritten against the real row.** The boundary that chose a table over a column was worth checking in production, not only in a firewall: zero membership rows versioned by the act.

**Weekly three.** *Learned:* the same fact can be correct in two different renderings at the same instant, and proving both simultaneously in production is what turns a privacy boundary from a claim into a measurement. *Turkish Rug House:* the company now records that a real person works in Engineering — visible on the Organization page, answerable by Heby, and told to the model without the person's email address. *Hebun AI:* one capability designed, built, validated, released, production-accepted and closed in a single loop; ledger 42→43; deployed commit identical to the release commit; one row, one audit event, zero unintended mutations across 59 tables; two full-suite runs and 638/638 on the second.

## OSA-4 — the people register: the capability the repository had already named four times

- **The highest-value idea was refused by released code, and the refusal was right.** A work TARGET DATE was the strongest first-pass candidate until `work-item.ts` was read: due date is listed there among the things "this authority has no mandate to hold". Re-opening a reasoned refusal to win a feature is exactly what CLOSED MEANS CLOSED forbids — so the loop re-ranked instead of arguing.
- **Four released modules had already written the gap down as a BOUND on themselves.** "it cannot enumerate the organization's people" (twice), "PLACEMENT REGISTER != MEMBER ROSTER", "keeps ONE item and no roster". A capability whose justification is quoted from the code it extends needs no invented activation condition.
- **`workforce` is the class that looks right and is not — and this was the sharpest case yet.** It is literally chartered for "the humans an organization is made of". It is also unconnected, declared only by a workspace the mock-surface gate withholds from a real tenant, and `/heby` resolves to Command — so filing durable membership rows behind it would have made them invisible to the only tenants that have any. **A class charter that matches your subject is not a reason to use a class nobody can reach.**
- **The privacy boundary was designed as a SELECT list, not a filter.** The register joins `users` for two eligibility conditions and projects not one of its columns. It cannot leak an address because it never selects one — proved against real rows, where the grounding JSON contains no `@` at all.
- **Enumeration is the gated act; records about yourself are not.** `readPlacementRegister` needs no Governance gate because it lists PLACEMENTS. This one lists PEOPLE, so it takes HLR's released gate, in HLR's released order: authority first, before any subject is looked at, so a refusal is never an oracle.
- **A refusal must not be answered as an empty list.** An organization holding no Governance authority gets `not-authorized` with a sentence saying it says nothing about who belongs — proved in postgres against a third tenant that has a real member.
- **The word-ban-on-honest-prose trap fired again, and again in code I had just written.** A firewall forbidding `memberships` in the projection tripped on the projection's own sentence counting memberships. Ban the SEAM — no schema import, no db handle, no query — never the vocabulary.
- **The pre-release review found what a green suite would have shipped.** A placement into a RETIRED department rendered as current on the new panel; the register's header named the legibility module by filename and thereby joined an exact consumer census as a consumer it is not.

**Weekly three.** *Learned:* the cheapest justification for a capability is the sentence a released module already wrote about what it cannot do — and the cheapest privacy control is a SELECT list that never names the column. *Turkish Rug House:* the Director can now open the Organization page and see who is actually in the company, with each person's placement beside them and "no department recorded" said out loud for anyone still unplaced — and Heby can answer it in Command without the model ever seeing an email address. *Hebun AI:* one capability designed, built, validated and released in a single loop with ZERO schema, zero migration, zero writer and ledger unchanged at 43; the 20th source class, Command only; nine released censuses moved together and stated; two full-suite runs and 641/641.

## OSA-4 production acceptance — the disagreement was predicted, then measured

- **The strongest evidence was written before the observation, not after.** Reading `users` in production showed `display_name` and `name` both NULL, so the page MUST render the email and Heby MUST say `name unavailable`. Stating that in advance turned the acceptance from "does it look right?" into "does production match the prediction?" — and it did, exactly.
- **G6D's durable answer-source evidence is the best corroboration instrument this repository has.** The Director's Heby answer left a stored row: `source_class people`, `record_ref member/55aa4d45-…`, `label name unavailable`, `authoritative true`, with the identifier and the exact membership timestamp in its detail. **Do not corroborate a model answer from its prose when the runtime stored the evidence set.**
- **A non-effects census has a blind spot shaped like its own timestamp column.** Scanning 59 tables `where created_at > …` silently skipped every table timestamped by `recorded_at` or `occurred_at` — including the evidence table that had just gained 45 rows. Six more tables scanned by their own column closed it. **A census is only as complete as the column it assumes.**
- **A row inside the window is not a row caused by the work.** `department_placements` gained one row in the last two hours — OSA-3's acceptance at 19:18:50Z, seventy-five minutes before this deployment was READY at 20:33:38Z. The deployment instant is what separates them, so fetch it and subtract rather than reasoning about "recent".
- **Four historical rows contain an email address and none of them are mine to fix.** One integrations account, and three `work` rows recorded before WORK-2's hardening reached production. Zero were written by this answer. **A permanent record cannot be tidied; it can only be stated.**
- **`audit_log` unchanged at 39 across the whole acceptance is the entire audit story.** A read writes nothing, and the cheapest proof is a count taken before the human acts.

**Weekly three.** *Learned:* predicting the exact production rendering from the authoritative columns BEFORE the human looks converts an acceptance into a measurement — and a database-wide non-effects scan must be built from each table's own timestamp column, not from the one you expected. *Turkish Rug House:* the company can now see who is actually in it, with placement beside each person, and ask Heby the same question — and production proved the model was told `name unavailable` while the Director's own screen said `senoltr@gmail.com`. *Hebun AI:* one capability designed, built, validated, released, production-accepted and closed in a single loop with ZERO schema, zero migration and zero writer; ledger unchanged at 43; deployed commit identical to the release commit; the 20th source class cited authoritatively in a real production answer; 122 table-scans for non-effects with every change named; two full-suite runs and 641/641.

## INT-5B2 — the stranded seam, and the pin that named its own successor

- **The best capability in the repository was one already built and never connected.** `readRepositoryPullRequests` had been proven against the real GitHub API by GITHUB-4 and had ZERO consumers since. Selection did not need a scoring framework — it needed `grep`, and then a check that the zero was still true today rather than remembered from a memory file.
- **A released pin asked, in writing, to be edited by whichever phase gave the seam a caller.** That is the cheapest possible handoff between milestones: not a roadmap entry, not a TODO, but an assertion that fails the moment the situation it describes changes.
- **The permission was already granted, and that is why this was one loop.** `pull_requests: read` has been in every minted token since GITHUB-4. A capability whose provider scope is already consented is a completely different size of work from one that needs a consent flow.
- **`maxProviderCalls: 2` was a released ceiling written for a different command, and it decided the design.** Reading it stopped a fan-out that would have quietly violated a stated budget; the answer was a SIBLING budget whose call count is DERIVED from the fan-out (`2 + 3N`) rather than typed next to it.
- **A module forbidden to mint an identifier is also forbidden to hold a clock.** A per-repository deadline computed with `Date.now()` tripped a released ban intended for identity minting. The ban was right anyway: one ceiling around the whole fan-out is simpler, and the trade — losing partial results on timeout — is the correct one, because a partial list under a timeout is the shape most likely to be read as complete.
- **The substring trap fired again, in my own firewall, on the word `patch` inside `dispatch`.** Ban the FIELD SHAPE (`.patch`, `"patch"`, `patch:`), never the bare word. Third time this pattern has cost a cycle; it is now the first thing to check when a structural ban fails.
- **A name ban was replaced by an EXACT list, which is stricter than what it replaced.** "The graph must not reach `readRepositoryPullRequests`" became "the graph reaches exactly these two GitHub readers and no other". Widening a boundary is acceptable when the replacement forbids more than the original.
- **Two bite-proofs failed for the right outcome and the wrong message.** An `assert.equal` without a message fires first and prints Node's generated text, so the bite's `expect` never matches. Give the assertion the sentence you want to read in the failure, then aim the bite at it.
- **A red suite was a NODE VERSION, not a regression.** `ama1-agent-mandate/bite-proofs` failed at clean `HEAD` with everything stashed under Node v20.20.2 and passed under v24.16.0 — the interpreter the previous loop's baseline ran under. Proving it twice (stash-to-HEAD, then version-to-version) cost two minutes and prevented editing somebody else's workstream to chase a phantom.

**Weekly three.** *Learned:* the highest-leverage work in a mature repository is often already written and merely unreachable — and the repository will usually tell you so in an assertion, if you read the pins rather than the roadmap. *Turkish Rug House:* the Director can now type `/pull-requests` in Heby and see what is actually open in the company's own GitHub repositories, with every line saying plainly that a pull request is GitHub's record and not the company's recorded work. *Hebun AI:* one capability selected, designed, built, validated and released in a single loop with ZERO schema, zero migration, zero writer, zero new command kind, zero new source class and zero new provider permission; the ledger unchanged at 43; ten bite-proofs all biting; four released censuses moved and stated; two full-suite runs and 644/644.

## INT-5B2 production acceptance — the empty answer was the hard one

- **Zero was the best possible acceptance case, not a weak one.** GitHub genuinely had no open pull requests, which is exactly the situation where a broken implementation and a correct one look identical on screen. What separated them was that the informational path and the unavailable path carry DIFFERENT provenance sentences — so the observed provenance proved which branch ran, not merely that something rendered.
- **Establish the provider's true answer from a second credential BEFORE the human acts.** `gh pr list` (the user's own OAuth token) and `gh api /orgs/.../repos` gave zero open PRs and exactly one repository. Hebun then said zero, and "Looked inside 1 repository". Corroboration by an independent credential beats corroboration by re-reading Hebun's own copy of the answer.
- **Verify rendered strings against the RELEASED COMMIT, not against memory of the source.** Every observed line was located in `44bd77c` at the exact branch that produces it. A transcription differed by one character (a semicolon for a colon) — worth naming as a transcription artifact rather than letting it read as a mismatch.
- **`integration_credentials` not moving is the proof that a token was never persisted.** KR-EXT1 learned the opposite lesson from Google, where a refresh silently raised the count. Here 18 → 18 across a live provider read means the installation token was minted and discarded inside the seam's callback frame, exactly as designed.
- **`heby_answer_source_evidence` not moving is the production proof of a DESIGN decision.** The capability declares no Heby source class; 757 → 757 across a live provider read is that decision measured rather than asserted. A count that does not move can be the strongest evidence in an acceptance.
- **A read must not re-verify a connection.** `integrations.version` stayed 3 and `updated_at` stayed equal to `last_verified_at` — so a successful provider read did not touch the lifecycle, just as a failed one could not have ended the grant. Reading is not verifying, and the timestamps are where that stops being a claim.

**Weekly three.** *Learned:* the emptiest possible provider answer is the most demanding acceptance, because only a design that keeps "nothing was returned" and "nothing was obtained" on different code paths can render it honestly — and the counts that DON'T move prove more than the ones that do. *Turkish Rug House:* the Director can now ask Heby what is open in the company's GitHub repositories and get GitHub's own answer, labelled as GitHub's, with the boundary against organizational work and organizational membership stated on the surface itself. *Hebun AI:* one stranded seam connected end to end in a single loop with zero schema, zero migration, zero writer, zero new command kind, zero new source class and zero new provider permission; ledger unchanged at 43; deployed commit identical to the release commit; six database counts unchanged across a live third-party read; 644/644 and 10/10 mutations bitten.

## DH-1 — the decision horizon: a third of an answer that looked like the whole of it

- **The gap was found by reading imports, not by imagining features.** `/approvals` imports one authority. Heby's `decision-records` projection imports one authority. Three authorities record decisions. That is the entire selection argument, and it took three greps.
- **A silent omission is worse than an absence.** Heby answered "what needs my decision?" with action requests alone and gave no hint that hypotheses and Knowledge versions were waiting elsewhere. A confidently complete-looking partial answer is the failure mode a Director acts on.
- **The class did not need widening — it needed reading.** `decision-records` was chartered as decision PREPARATION material, never as "action requests"; its first reader just happened to be the only authority connected then. Re-reading a class charter beat inventing a class: census unchanged at 20.
- **Composition means consuming the released projection, not the seam under it.** The action half travels verbatim from `readDecisionQueueGroundingSource`, which carries far richer per-item detail than a horizon row. A firewall asserts the horizon never reaches past it — otherwise "composing" would quietly have meant "re-deriving with less".
- **The completeness verdict is the whole feature.** Three sources means three ways to accidentally say "nothing is waiting" during an outage. It is enforced in the read model, in the projection's gate on the empty sentence, and again in the surface — and every one of those three was bite-proofed.
- **A `sources` narrowing option is safe only if the verdict is computed over the sources ASKED.** The Heby projection skips the action source (it already has it, richer) and then accounts for it itself. A completeness verdict that ignored the narrowing would claim completeness about a source it never asked.
- **A pre-existing import cycle nearly made a pure test impossible.** `read-action-authorizations` throws when loaded first in a process; released tests hide it by importing `db/client.server` first. The horizon orders its own imports to load cleanly and RECORDS that it is accommodating somebody else's cycle rather than fixing it — naming a workaround as a workaround is what stops it becoming folklore.
- **Three bite-proofs failed for the right outcome and the wrong message, again.** Every time, the assertion that fired first was the stronger one. Aim the bite at what actually defends the property, not at the sentence you happened to write near it.
- **The runtime was pinned BEFORE the first suite run, not after.** Last loop lost a cycle to Node v20 vs v24; this loop identified v24.16.0 as the baseline interpreter during preflight and both suites ran on it. A precheck that costs one command prevents a phantom regression hunt.

**Weekly three.** *Learned:* the most valuable gaps in a mature system are the ones where an existing answer is CORRECT but PARTIAL — and the fix is a completeness verdict, not more data. *Turkish Rug House:* the Director now opens `/approvals` and sees everything the company has recorded as needing a decision — actions, agent hypotheses and unreviewed Knowledge — each labelled with who owns it and where it is decided, and told plainly when the list could not be completed. *Hebun AI:* one capability selected, designed, built, validated and released in a single loop with zero schema, zero table, zero writer and zero new source class; ledger unchanged at 43; ten bite-proofs bitten; two censuses moved and stated; two full-suite runs and 647/647 on the verified baseline runtime.

## DH-1 production acceptance — the record answered what the screen could not

- **A UI that cannot scroll is not a failed acceptance.** The Heby surface truncated the tail of the answer, and the missing part — the completeness verdict — was recovered from G6D's stored answer-source evidence exactly as served. Corroborating a model answer from the durable evidence set beats corroborating it from a screenshot, and this time it was the ONLY way.
- **Predict the answer from the authorities BEFORE the human looks, and production hands you a better case than you would have designed.** Three pending actions plus one approved, an empty hypothesis table, and two Knowledge versions of which one was ratified — a mixed horizon that exercised the subtraction, the answered-with-nothing branch and the complete verdict in a single observation.
- **The strongest single line of evidence was an ABSENCE inside a present record.** `65f7f57f…` is ratified and does not appear; the `approved` action request does not appear. A list that is right about what it OMITS is worth more than one that is right about what it contains.
- **`3 → 4` is the whole capability, stated as a number.** Before DH-1 both `/approvals` and Heby answered 3. The organization actually had 4 things waiting, across three kinds. The gap was never a missing feature — it was a confident answer that was one third of the truth.
- **"Gathering is not deciding" is provable in one query.** Every source's state unmoved: 3 pending, 1 approved, 0 hypotheses, 2 nodes, 1 knowledge decision, 6 decision records, audit 39 → 39 with an empty window. A read model that touches four authorities is exactly where that proof is worth writing down.

**Weekly three.** *Learned:* when the interface cannot show you the end of an answer, the durable evidence the runtime already stores is a better witness than the screen ever was — and an acceptance designed as a prediction turns an observation into a measurement. *Turkish Rug House:* the Director now sees all four things the company has recorded as needing a decision in one place, each labelled with the authority that owns it and the surface where it is taken, and is told explicitly when the list could not be completed. *Hebun AI:* one capability selected, built, validated, released, production-accepted and closed in a single loop with zero schema, zero table, zero writer and zero new source class; ledger unchanged at 43; deployed commit identical to the release; every decision authority's state unmoved across the acceptance; 122 table-scans with every change named; 647/647.

## LM-1 — the map that said Hebun had no authority to list its own people

- **A projection's own contract is a to-do list with an expiry date.** Live Map said the human→department relationship was "owned and this projection has nowhere to draw it", and separately that Hebun had "no authority that lists them". OSA-4 made the second sentence FALSE the day it shipped, and nothing failed — prose does not have a test. **A stale claim on a trusted surface is a defect, not documentation debt.**
- **FAST path is about the boundary, not the file count.** No schema, no writer, no shared primitive — so no full suite. But five released contracts encoded the exact claim being changed, and every one had to be run and repointed. That is rigor, not ceremony, and it is why this ran past the 45-minute target.
- **A released guard found the real risk before I did.** `map-surface` asserts "membership is never relabelled as placement" — a property that matters MORE once both are drawn. The distinction now travels on the node and in both people sentences instead of only in the absence sentence it used to live in.
- **Rendering absence and rendering presence are different components.** `AbsentDomain` returns null when a domain is `available`, so making departments and people available would have rendered NOTHING. Adding a state to a domain is not the same as drawing it.
- **A second relation kind broke a single-edge assumption silently.** The canvas printed `edges[0].basis` — correct while one relation existed, and quietly misleading the moment a second was drawn. **When you add the second of anything, grep for code that assumed there was one.**
- **The honest-prose trap fired again, on comments recording the repair itself.** Scan the SHIPPED CONSTANTS, never the file that explains them. Fourth occurrence; it is now reflexive.
- **Two bite-proofs pointed at code this milestone replaced.** Both were re-aimed at the same guarantee one state along — an unread register downgraded to a measured zero, and a basis replaced by "These are related." A mutation whose find-string is gone proves nothing and must be repointed, never deleted.

**Weekly three.** *Learned:* when an authority ships, every surface that once said "nobody owns this" becomes a liability — and only the ones with tests will tell you. *Turkish Rug House:* the Live Map now draws the company's departments and its people, with a line from each person to the department they are recorded as working in, and says plainly that membership is not employment and a missing line is not a missing job. *Hebun AI:* one FAST capability — zero schema, zero writer, zero new authority, no Heby exposure; two new node kinds and the second relation the map ever proved; five released contracts repointed and every one re-run; ledger unchanged at 43.

## LM-1 production acceptance — proportional, and it held

- **The non-effects sweep was four tables, not 122, and that was the right call.** The capability contains no writer, so the credible boundary is the three source tables it reads plus the audit log. Proving the whole database unchanged after a read projection is ceremony; proving the authorities it draws from were not touched by drawing them is the measurement.
- **Predicting the map from the authorities made the observation a check, not an interpretation.** Engineering, `senoltr@gmail.com`, one `works-in` edge — written down before the page was opened, matched line for line.
- **The Director's report named the truth semantics back**, which is the strongest signal the surface carries them: works-in grounded in `department_placements`, not a role or reporting line, and membership held apart from employment. Those sentences exist on the node precisely so a reader repeats them instead of inferring an org chart.

**Weekly three.** *Learned:* proportional acceptance is a measurement of the boundary that changed — a read projection with no writer earns a four-table sweep, and spending more proves nothing new. *Turkish Rug House:* the company's map now shows Engineering, shows the one person in it, and draws the line between them from the record that owns it. *Hebun AI:* one FAST capability closed with zero schema, zero writer and no full suite; ledger unchanged at 43; deployed commit identical to the release; five released contracts repointed and re-run instead of a global rerun.

## ORG-1 — the department nobody could see whole

- **The best FAST capability is usually a question, not a fact.** Every input existed; what was missing was the shape a Director thinks in — "what IS Engineering?" Four authorities, four answers, one grouping, zero new facts.
- **Adjacency is the most dangerous thing a composition can render.** Putting a department's PEOPLE and its WORK on one card invites exactly one wrong inference, and WORK-1/WORK-2 had already refused it in words. The denial went on the department itself, rendered only where both halves are present — which is the only place the wrong reading is available.
- **An empty slice is not a zero.** Filtering a register down to one department produces an empty array for "nothing here" AND for "the register was truncated before it got here". The panel computes `placementsComplete` / `workComplete` once and refuses to say zero without them.
- **A reasoned placement is not a gap.** `integrations` is declared by the `platform` workspace and only there, with INT-5A's stated reason. Moving it to Command would have been convenient and would have reopened a reasoned refusal — the strongest candidate I rejected this loop, and rejecting it took two minutes because the reason was written down at the time.
- **Sorted exact lists cost a cycle every single time.** Third loop running where a census entry landed one line off and the diff was pure ordering. `department-composition` < `department-structure` < `departmental-placement`, because `-` sorts before `a`.
...
- MEDIA-SUPPLIED (2026-09-25): PostgreSQL caps a regex repetition bound at 255 — a CHECK like `~ '^[A-Za-z0-9_-]{1,256}$'` is emitted by drizzle-kit without complaint and only fails at insert time ("invalid repetition count"). Put length in its own `char_length(...) between` predicate; only a real disposable-Postgres insert catches it.
  Q1 learned: a non-generated origin fits the Media authority as typed provenance columns on `media_assets` (PUBLISH-0's derived precedent), not as a fake invocation. Q2 TRH: real product photos from Drive can reach the governed publish chain without being mislabelled as AI output. Q3 Hebun: every tenant gets "bring your own photo" through the same verification, storage and lineage as generated media.
- MEDIA-SUPPLIED PICKER CORRECTION (2026-09-25): a new provider-backed door must reuse the ACCEPTED permission model, not the first capability that works — `drive.readonly` "worked" and silently reversed the Picker + `drive.file` decision. Also: bite-proof tests mutate tracked sources mid-suite (a migration .sql showed as modified); never stage or audit scope while the suite runs.
