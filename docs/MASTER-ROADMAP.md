...
**Opened by explicit Director decision** (§18, rule 12), recorded in §13 and §20. It is **OPEN** in the sense §9 defines it. The Director selected **Agent Mandate Authority** as its first program; **AMA-1, AMA-2 and AMA-3 are RELEASED and AMA-4 is PRODUCTION-ACCEPTED**, so the **Agent Mandate program is CLOSED** (§13, §20) — the authority records a ceiling, enforces it on agent-originated proposals, is reachable by a human, is reportable by Heby, and has now been exercised end to end against the real control plane. The era's outcome sentence above is its exit criterion and is not restated elsewhere. **A closed program is not a closed era**: nothing about Era III's outcome has been measured, and no AMA milestone satisfies any of it. The §13 constraints are unrelaxed. **No next program is selected** — that is a separate Director decision, taken from measured reality and never from numbering.
...
| **Agent Mandate Authority — AMA-4** (fourth Era III milestone) | **PRODUCTION-ACCEPTED** — the Agent Mandate Authority is exercised end to end against the real control plane. **The AGENT MANDATE PROGRAM is CLOSED.** | An **acceptance** milestone, not an architecture one: no authority created, no owner moved, and the mandate gate stayed exactly where AMA-2 put it. Production was measured **behind by one migration** and the released `platform:migrate` ceremony was run **by the Director at a TTY** (it refuses piped stdin); ledger **39 → 40**, verdict `pending` → `converged`, digest `2a9522bb…`, organizational counts byte-identical across it. Five human product acts proved the chain: an **empty** ceiling recorded (revision 1), an agent proposal refused **`action-outside-agent-mandate`** with no row written, a revision to `["send"]` (revision 2, revision 1 immutable), a real proposal **filed and left `pending`**, and Heby grounding on `agent-mandate` — the **first such evidence rows ever written** in production. The empty ceiling came first **because it had to**: `AGENT_ORIGINABLE_ACTION_KINDS` is `["send"]`, so `[].some(...)` is the only route to an out-of-mandate refusal — the revision was structurally required, not manufactured. **One real defect at the gate**, repaired at `3bdcf27`: the durable record collapsed all three mandate refusals into `not-authorizable`, so the cause had to be established by elimination; the inlet now carries the writer's `ActionRequestRefusal` verbatim and origination prefers it, with **no new vocabulary, no schema and no UI change**. AGENT-PROPOSAL-4B's pin was repaired **STRICTER**. Final delta: `agent_mandates` 0 → 2, decisions 4 → 6, sessions 4 → 6, audit 31 → 35, requests 3 → **4**, **permits 1 → 1**, **execution attempts 1 → 1**, `agents` byte-unchanged, `authority_ceiling` still NULL with no writer. **No permit, no execution, no provider send**; the filed proposal is deliberately still `pending`. **618/618.** Closure: `docs/product-vision/runtime/hebun-ama4-production-acceptance-closure.md`. |
| Heby decision-queue grounding (read-only seam, not a program) | **RELEASED at `c172aaf` and PRODUCTION-ACCEPTED.** Heby can state what is awaiting a human decision. **No new authority, ZERO schema, ledger unchanged at 40.** | A read-only grounding seam, deliberately not an architecture program. Phase 0 resolved the one open ambiguity by repository semantics rather than by name: `decision-records` legitimately means **decision-PREPARATION material** — its own contract says so, both declaring workspaces (`command`, `decisions`) also declare the `decision-preparation` capability, and the `decisions` profile routes to `/approvals` and states *"Heby prepares; it never approves, rejects, or authorizes."* So the class was connected to the truth it was always about, and nothing was folded into `governance`, which reads the constitutional record a decision is taken UNDER. **One projection**, living inside the authority that owns the rows (`action-authorization/heby-decision-queue-source.server.ts`), consuming the released `readPendingActionRequests` — the same seam `/approvals` has read since R3A. **No second reader and no second queue.** PENDING ONLY: approved, permitted and executed stay with `recorded-acts`, and **no elapsed time is computed** because E2-4's attention observation already owns duration and a second clock is how two surfaces come to disagree. **No priority, urgency, risk score or recommendation** — no authority owns any of them, so nothing may assert one. Three answers kept apart, and the dangerous pair pinned in both directions: **UNAVAILABLE != EMPTY QUEUE**, because a Director told "nothing needs your decision" during an outage acts on it by doing nothing. The firewall is **WALKED, not asserted**: the value-import closure contains zero durable writers and reaches none of the decision writer, proposal writer, permit consumer, permit revoker, executor, send transport, Governance decision authority or mandate writer — four of which are files in the SAME DIRECTORY — and it self-tests by proving the decision writer IS detected as a writer. APP-1's one-reader census fired and was **repaired STRICTER**: its "holds no queue of its own" proof was hardcoded to a single path, so the list could have grown while only the original entry was ever checked; it now measures EVERY non-route consumer. **620/620.** **PRODUCTION ACCEPTANCE PASS**, on the full `/heby` Command workspace: Heby listed exactly the **3 pending** proposals, **excluded the 1 approved** one from pending truth, stated each is irreversible and carries `CONSEQUENTIAL_MUTATION`, and said all three *"remain pending until the Director decides on the approvals surface"* — **no recommendation and no advocacy**. Production mutation: **NONE** (requests 4, permits 1, attempts 1, decisions 6, mandates 2, audit 35 — unchanged). **The single-owner duration semantics HELD, and were measured rather than assumed.** Heby said the oldest had waited "approximately 3 days and 22 hours"; the turn's own evidence shows that came from E2-4's `operations` item — `3 awaiting · oldest waiting 3d 22h · basis action-request.created_at`, `authoritative: false` — already present in the same grounding context. This class carried no duration and computed none, which is exactly why one number reached the model instead of two. **One measured cosmetic limitation, corrected deliberately NOT:** Heby printed raw proposal UUIDs, which come from `ResolvedSourceItem.recordRef` and are **not displayed anywhere on `/approvals`**, so they are unmatched for the human workflow. No change was made — `recordRef` is contractually the retrieval layer's stable identity ("the model can never invent this") and weakening it to tidy one answer would cost evidence binding, while instructing the model not to print it is prompt-shaping. Target label, action kind and filed time already identify each item to a human. |
| Governed Delivery Reconciliation — **GDR-0** provider truth discovery | **DISCOVERY COMPLETE — read-only. Verdict B: REDESIGN / DEFERRED. GDR is NOT an active program.** | Ran against `c177736`, read-only: no provider call was made, no credential was read, and no repository file changed. Finding: **meaningful post-acceptance provider truth exists** — `GET /emails/{id}` returns `last_event` over eleven event types (`delivered`, `bounced`, `complained`, `delivery_delayed`, `suppressed`, …), so this is genuine delivery state and not send metadata. But the previously suggested **polling + one-new-table model is REJECTED**, on two measured facts: Resend exposes **no read-only credential scope** — only `full_access` ("create, delete, get, and update any resource") and `sending_access` ("can only send emails") — so polling would trade a correctly-narrow send key for one that can delete and update every resource in the account; and provider retention is **short and plan-bounded**, which makes polling lossy by construction. **Signed event ingestion (Svix headers, per-endpoint secret, at-least-once, order NOT guaranteed) is the stronger future architecture**, and it is **not justified now**: it would introduce Hebun's **first public unauthenticated inbound endpoint** against a measured product need of one recipient and one execution. A real provider read was **NOT performed and NOT improvised** — `ExternalSendAdapter` declares exactly one operation and `ADAPTER_SANDBOX_BOUNDARY` states "exactly one outbound operation is expressible", so a Resend read is architecturally unexpressible today; the production key's permission is therefore **UNVERIFIED**. The pins hold and are unchanged: **ACCEPTED != DELIVERED**, **DELIVERED != HUMAN READ/RECEIPT**, **DELIVERED != ORGANIZATIONAL SUCCESS**. **Recipient count alone is NOT the activation trigger.** Reconsider GDR when delivery truth becomes necessary for a real product workflow, an operational requirement, agent evaluation, a compliance requirement, or a measured organizational blind spot. |
...
Opening the era selected no milestone and authorized no implementation. **That decision was then taken separately**: the Director selected **Agent Mandate Authority** as the first Era III program, and **AMA-1 — Authority Foundation — AMA-2 — Proposal Enforcement — AMA-3 — Product Surface and Heby Grounding — are all RELEASED, and AMA-4 — Production Acceptance — is PRODUCTION-ACCEPTED** (§20). **The Agent Mandate program is CLOSED.** Era III's state is therefore **OPEN**: one program closed, no program active, and an era outcome nobody has measured.

**AMA-4 closed the program by exercising it, not by extending it.** A human recorded an empty ceiling, watched an agent-originated proposal be refused `action-outside-agent-mandate` with no row written, revised the ceiling to `["send"]`, filed a real proposal that stopped at `pending`, and asked Heby what it was for. The empty ceiling had to come first: `AGENT_ORIGINABLE_ACTION_KINDS` is `["send"]`, one kind, so "outside the mandate" is reachable **only** through an empty scope — the revision was structurally required, not manufactured. One real defect was found at the acceptance gate and repaired: the durable record collapsed all three mandate refusals into `not-authorizable`, which is exactly what `action-authorization/contracts.ts` says may never happen. Closure: `docs/product-vision/runtime/hebun-ama4-production-acceptance-closure.md`.
...
```
ERA III OPEN          != ERA III NEAR CLOSING
ONE PROGRAM CLOSED    != THE ERA OUTCOME IS MEASURED
PROGRAM CLOSED        != NEXT PROGRAM SELECTED
MANDATE RECORDED      != PROPOSAL-ENFORCED
MANDATE RECORDED      != HEBY-GROUNDED
MANDATE RECORDED      != PRODUCTION-ACCEPTED
PRODUCTION-ACCEPTED   != AGENTS FULLY BUILT
PRODUCTION-ACCEPTED   != A SECOND AGENT EXISTS
REFUSED               != COSTLESS
MANDATE               != AUTHORITY
MANDATE               != PERMISSION
MANDATE               != CAPABILITY
...
### AMA-1 RELEASED — the Agent Mandate Authority exists, and it enforces nothing

**Agent Mandate Authority is the first Era III program**, selected by Director decision after the
boundary discovery it was gated on. **AMA-1 — Authority Foundation — is RELEASED.**

The discovery's recommendation was implemented as given: a **dedicated authority** owning a
versioned, agent-bound sibling table, with Governance authorizing every transition through the
transaction-joinable decision writer and owning none of the state. **Governance gained a ninth
subject type and no mandate-state writer**, and a firewall asserts no file under
`governance-decision/` can name the mandate table.

| | |
|---|---|
| Table | `agent_mandates` — one, additive. **No existing table altered**; `agents` byte-unchanged |
| Migration ledger | **39 → 40** — the first Era III schema. Production stands at 39 until its own gated ceremony |
| Enum | one value: `governance_domain` gains `agent-mandate`. The unused `agent-registration` was **refused** — registration is an agent coming into existence, which `agent-identity` owns |
| Governance subject | `agent_mandate`. **No migration** — `decision_records.subject_type` is `text` |
| Writer / read seam / audit sibling | one each. The audit sibling is the **seventh**, over the same shared `audit_log` sink |
| Human-only CHECKs across the database | **8 → 9** — `agent_mandates_human_establisher_chk` |
| Validation | **612/612**, typecheck clean, lint zero errors, **12/12 bite-proofs bite** |

**The ceiling is a TYPE, not a check.** A mandate's scope is `AgentOriginableActionKind[]` — the
released origination vocabulary itself, the same reference rather than a copy — so a superset does
not compile. The table repeats the list once in SQL, because a CHECK cannot import TypeScript, and a
test pins the two equal so the only possible drift direction is the database being **narrower**.

**One revision is EFFECTIVE, and nothing is ever updated.** Effective is `max(mandate_revision)`,
enforced by a UNIQUE ordinal rather than a partial index over an effective flag — because a partial
index would require the writer to **edit the previous row** on every change, and a historical record
a superseding write can edit was never a record. Withdrawal is an **empty scope**, not a boolean:
`NO MANDATE != EMPTY MANDATE`. The unique ordinal is also the concurrency guarantee, which is why
this writer needs no table lock where the agent identity ceremony does.

**An agent cannot establish or widen its own mandate**, for two independent reasons: there is no
agent authentication in Hebun, and the establisher column carries a CHECK admitting only `human`. A
mandate for another tenant's agent is refused by a composite foreign key, not by a predicate.

**`agents.authority_ceiling` is never named** — not in the feature, the schema, the audit sibling or
the migration. It has no writer and **does** have a reader that summarizes it into canonical actor
resolution, so filling it would publish a constraint as an authority with nothing else failing. The
boundary discovery found that hazard; a bite-proof now defends it.

**AMA-1 ENFORCES NOTHING, and the absence is measured rather than promised.** A census over all of
`src/` proves exactly three modules outside the feature can see a mandate — the schema barrel, the
audit sibling and the Governance decision writer — and none is a proposal path.
`AGENT_ORIGINABLE_ACTION_KINDS` is untouched, `/agents` is unchanged, no surface renders a mandate,
and every audit row carries `enforced: false`.

**Two real regressions were found by the suite and fixed, not worked around**: 26 absolute
migration-ledger pins, and the human-only CHECK census. Every one was **extended, never relaxed** —
loosening an enumeration to "at least N" would let a future phase delete a released constraint and
...
AMA-1 RELEASED     != AMA-2 AUTHORIZED
ONE PROGRAM ACTIVE != THE ERA OUTCOME IS MEASURED
